...
- **待爬列表**：筛选框内新增独立的 `Pending Crawl List / 待爬列表` 操作行；点击只通过只读 GET 展示当前待爬题面与题解，弹窗内确认 `Rebuild / 重建` 后才会 POST 并启动爬取。

### 修复

- **题面全量就绪**：合法空 `.sample-test` 不再误判，旧式直属 `<br>` 仅作样例分隔；资源按 magic bytes 选择扩展名，支持上游错误图片子类型与严格 BMP 结构/调色板校验（兼容已证实的文件长度多报 1 字节）。正常增量重爬后 `11363/11363` 题面 ready、status marker 清零。
- **题解抓取恢复**：错误页识别不再被正文中的 `does not exist` 等普通文本误触发；合并场题解使用精确当前比赛题号取得 CSRF，别名题号回退到比赛页但保持源 `problemCode` 不变。共恢复并发布 101 场题解，当前为 1786 ready、185 confirmed absent、34 个资源失效导致的可重试项。
- **资源与缓存安全**：资源路由以 descriptor-relative `O_NOFOLLOW` 逐级打开，拒绝缓存根、父目录、assets 目录及最终文件的符号链接；BMP 拒绝压缩、截断、负宽、无调色板及非法颜色数。
- **爬取任务交接**：活动任务期间确认 Rebuild 会在当前 writer 结束后执行一次延迟重试；修复已尝试失败项丢失、进度提前显示完成、线程启动失败丢请求、递归恢复、旧错误覆盖新任务及启动更新被跳过等竞态，同时保持单 writer。

### 性能

- **待爬预览提速**：预览改为读取原子发布的文档文件名与状态标记，不再反序列化和校验全部 13142 份 ready 文档；实测接口从约 3.4 秒降至约 0.16 秒，POST 仍会重新评估最新快照，实际 crawler 默认仍执行完整文档校验。
...
- **增量 Rebuild、预览与主题弹窗**：网页 Rebuild 明确只重试当前跳过的题面和题解；确认前通过只读预览接口在两个可滚动 Tab 中分别显示数量与可点击项目，确认后把完整快照重新加入对应队列并清空列表；并入正在执行的启动爬取而不创建重复 writer，所有反馈统一使用随 Gruvbox/Catppuccin 变化的浮动窗口。
- **阅读器点击优先爬取**：打开可重试的缺失题面或题解会通过独立 POST 将其移到对应内存队列前端；题面下一项生效，题解下一批生效，无活动任务时立即启动增量爬取，同时保持所有内容 GET 只读。
...
- **公式占位保护（上下标替换不误伤公式）**：`$a^k$` 等公式里的 `^`（LaTeX 幂）被上标替换正则跨公式误匹配（2246 题解公式渲染错乱——`<sup>` 嵌进公式）→ 替换前公式段（`$...$`/`$$...$$`）与代码块同级占位保护；全库扫描 13163 文件验证：8117 个含公式文件零破坏、56515 处上下标正确还原、占位符零残留
