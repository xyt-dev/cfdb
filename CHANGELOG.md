...
- **孤立 `**` 残留**：CF 老博客用 `<b><br/></b>` 当分隔（空粗体标签残缺）→ 转换后出现行尾/单独行/行首孤立 `**`（如 2.md 的 `Lets start from the end.**`）→ 转换器 `_fix_broken_bold` 按行修复（奇数 `**`：单独行删除、行尾删除、行首删开标记；配对粗体与 `2**n`/`(**)` 字面记号不受影响）；12 个文件 64 行已清洗
