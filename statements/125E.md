...
The first input line contains three integers *n*, *m*, *k* (1 ≤ *n* ≤ 5000;0 ≤ *m* ≤ 10^5^;0 ≤ *k* < 5000), where *n* is the number of cities in the country, *m* is the number of roads in the country, *k* is the number of capital roads in the required set. Then *m* lines enumerate the roads in question. Each road is specified by three numbers *a*~*i*~, *b*~*i*~, *w*~*i*~ (1 ≤ *a*~*i*~, *b*~*i*~ ≤ *n*; 1 ≤ *w* ≤ 10^5^), where *a*~*i*~, *b*~*i*~ are the numbers of cities linked by a road and *w*~*i*~ is its length.
