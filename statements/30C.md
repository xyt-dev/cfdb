...
The first line contains integer *n* (1 ≤ *n* ≤ 1000) — amount of targets in the shooting gallery. Then *n* lines follow, each describing one target. Each description consists of four numbers *x*~*i*~, *y*~*i*~, *t*~*i*~, *p*~*i*~ (where *x*~*i*~, *y*~*i*~, *t*~*i*~ — integers,  - 1000 ≤ *x*~*i*~, *y*~*i*~ ≤ 1000, 0 ≤ *t*~*i*~ ≤ 10^9^, real number *p*~*i*~ is given with no more than 6 digits after the decimal point, 0 ≤ *p*~*i*~ ≤ 1). No two targets may be at the same point.
