...
Next *n* - 1 lines contain descriptions of roads of the Nvodsk, each line contains three integers *a*~*i*~, *b*~*i*~, *c*~*i*~ (1 ≤ *a*~*i*~, *b*~*i*~ ≤ *n*, 0 ≤ *c*~*i*~ ≤ 10^9^, *a*~*i*~ ≠ *b*~*i*~) — junctions *a*~*i*~ and *b*~*i*~ are connected with a street whose beauty equals *c*~*i*~.
