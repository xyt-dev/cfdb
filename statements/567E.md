...
Next *m* lines contain the roads. Each road is given as a group of three integers *a*~*i*~, *b*~*i*~, *l*~*i*~ (1 ≤ *a*~*i*~, *b*~*i*~ ≤ *n*; *a*~*i*~ ≠ *b*~*i*~; 1 ≤ *l*~*i*~ ≤ 10^6^) — the cities that are connected by the *i*-th road and the time needed to ride along it. The road is directed from city *a*~*i*~ to city *b*~*i*~.
