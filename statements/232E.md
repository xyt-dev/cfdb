...
The next line contains integer *q* (1 ≤ *q* ≤ 6·10^5^) — the number of queries. Next *q* lines contain four space-separated integers *x*~1~, *y*~1~, *x*~2~ and *y*~2~ (1 ≤ *x*~1~ ≤ *x*~2~ ≤ *n*, 1 ≤ *y*~1~ ≤ *y*~2~ ≤ *m*) — the coordinates of the starting and the finishing cells. It is guaranteed that cells (*x*~1~; *y*~1~) and (*x*~2~; *y*~2~) are white.
