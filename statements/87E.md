...
The first line contains an integer *n*, which represent the number of the polygon's vertexes (3 ≤ *n* ≤ 5·10^4^). Next *n* lines contain two integers *x*~*i*~, *y*~*i*~ each, they are the coordinates of the polygon's *i*-th vertex in the counterclockwise order.

After the cities' description follows the integer *m* (1 ≤ *m* ≤ 10^5^), which represents the number of hills. Next *m* lines each contain two integers *x*~*j*~, *y*~*j*~, they are the coordinates of the *j*-th hill.
