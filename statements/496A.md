...
There are *n* holds on the wall, *i*-th hold is at height *a*~*i*~ off the ground. Besides, let the sequence *a*~*i*~ increase, that is, *a*~*i*~ < *a*~*i* + 1~ for all *i* from 1 to *n* - 1; we will call such sequence a track. Mike thinks that the track *a*~1~, ..., *a*~*n*~ has difficulty
