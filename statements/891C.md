...
The *i*-th of the next *m* lines contains three integers *u*~*i*~, *v*~*i*~, *w*~*i*~ (*u*~*i*~ ≠ *v*~*i*~, 1 ≤ *w*~*i*~ ≤ 5·10^5^) — the endpoints and weight of the *i*-th edge. There can be more than one edges between two vertices. It's guaranteed that the given graph is connected.

The next line contains a single integer *q* (1 ≤ *q* ≤ 5·10^5^) — the number of queries.

*q* lines follow, the *i*-th of them contains the *i*-th query. It starts with an integer *k*~*i*~ (1 ≤ *k*~*i*~ ≤ *n* - 1) — the size of edges subset and continues with *k*~*i*~ distinct space-separated integers from 1 to *m* — the indices of the edges. It is guaranteed that the sum of *k*~*i*~ for 1 ≤ *i* ≤ *q* does not exceed 5·10^5^.
