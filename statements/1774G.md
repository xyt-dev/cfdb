...
The $i$-th of the next $m$ lines contains two integers $x_i$ and $y_i$ ($1 \leq x_i \lt y_i \leq 10^9$), denoting a segment $[x_i, y_i]$.

It is guaranteed that all segments are distinct. More formally, there do not exist two numbers $i, j$ with $1 \le i \lt j \le m$ such that $x_i = x_j$ and $y_i = y_j$.

The $i$-th of the next $q$ lines contains two integers $l_i$ and $r_i$ ($1 \leq l_i \lt r_i \leq 10^9$), describing a query.
