...
The first input line contains two integers *n* and *m* (1 ≤ *n*, *m* ≤ 10^9^) — the yard's sizes. The second line contains integers *x*~*c*~ and *y*~*c*~ — the initial square's coordinates (1 ≤ *x*~*c*~ ≤ *n*, 1 ≤ *y*~*c*~ ≤ *m*).

The third line contains an integer *k* (1 ≤ *k* ≤ 10^4^) — the number of vectors. Then follow *k* lines, each of them contains two integers *dx*~*i*~ and *dy*~*i*~ (|*dx*~*i*~|, |*dy*~*i*~| ≤ 10^9^, |*dx*| + |*dy*| ≥ 1).
