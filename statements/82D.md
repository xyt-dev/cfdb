...
The first line of the input file contains a single number *n* (1 ≤ *n* ≤ 1000), which is the number of people in the sequence. The second line contains space-separated integers *a*~1~, *a*~2~, ..., *a*~*n*~ (1 ≤ *a*~*i*~ ≤ 10^6^). The people are numbered starting from the cashier to the end of the queue.
