...
The second line contains *n* integers *a*~*i*~ (|*a*~*i*~| ≤ 10^9^) — the initial report before it gets to the first manager.

Then follow *m* lines with the descriptions of the operations managers are going to perform. The *i*-th of these lines contains two integers *t*~*i*~ and *r*~*i*~ (

![](images/631C_1.png)

, 1 ≤ *r*~*i*~ ≤ *n*), meaning that the *i*-th manager sorts the first *r*~*i*~ numbers either in the non-descending (if *t*~*i*~ = 1) or non-ascending (if *t*~*i*~ = 2) order.
