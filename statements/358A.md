...
The first line contains a single integer *n* (1 ≤ *n* ≤ 10^3^). The second line contains *n* distinct integers *x*~1~, *x*~2~, ..., *x*~*n*~ ( - 10^6^ ≤ *x*~*i*~ ≤ 10^6^) — the *i*-th point has coordinates (*x*~*i*~, 0). The points are not necessarily sorted by their *x* coordinate.
