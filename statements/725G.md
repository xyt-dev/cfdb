...
The next *m* lines describe the messages. The *i*-th of them contains two integers *x*~*i*~ and *t*~*i*~ (1 ≤ *x*~*i*~ ≤ *n*, 1 ≤ *t*~*i*~ ≤ 10^9^) — the number of the vertex of the initiator of the *i*-th message and the time of the initiation (in seconds). The messages are given in order of increasing initiation time (i.e. *t*~*i* + 1~ ≥ *t*~*i*~ holds for 1 ≤ *i* < *m*). The pairs (*x*~*i*~, *t*~*i*~) are distinct.
