...
The first line contains an integer *n* (1 ≤ *n* ≤ 2·10^5^) which represents the number of houses in the village. The second line contains the coordinates of houses — the sequence *x*~1~, *x*~2~, ..., *x*~*n*~ of integer numbers (1 ≤ *x*~*i*~ ≤ 10^9^). It is possible that two or more houses are located on one point. The coordinates are given in a arbitrary order.
