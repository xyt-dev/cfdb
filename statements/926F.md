...
The *i*-th of the following *n* lines contains two integers *d*~*i*~ and *t*~*i*~ (1 ≤ *d*~*i*~ ≤ *m*, 1 ≤ *t*~*i*~ ≤ 10^9^) — the index of the day when Arkady will make the *i*-th top up, and the amount he will deposit on this day. It is guaranteed that the indices of the days are distinct and are given in increasing order, i. e. *d*~*i*~ > *d*~*i* - 1~ for all *i* from 2 to *n*.
