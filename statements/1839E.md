...
- Third round: the first player chooses $i = 4$. There is no $j \neq 4$ such that $a_j \gt 0$, so the second player can't make a correct move and the first player wins. Jury's program prints $j = 0$. After reading it, contestant's program terminates.
