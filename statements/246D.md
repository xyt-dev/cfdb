...
The first line contains two space-separated integers *n*, *m* (1 ≤ *n*, *m* ≤ 10^5^) — the number of vertices end edges of the graph, correspondingly. The second line contains a sequence of integers *c*~1~, *c*~2~, ..., *c*~*n*~ (1 ≤ *c*~*i*~ ≤ 10^5^) — the colors of the graph vertices. The numbers on the line are separated by spaces.
