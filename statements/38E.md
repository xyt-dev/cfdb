...
The first input line contains an integer *n* (1 ≤ *n* ≤ 3000) which is the number of marbles. The next *n* lines contain the descriptions of the marbles in pairs of integers *x*~*i*~, *c*~*i*~ ( - 10^9^ ≤ *x*~*i*~, *c*~*i*~ ≤ 10^9^). The numbers are space-separated. Each description is given on a separate line. No two marbles have identical initial positions.
