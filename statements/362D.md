...
Next *m* lines describe the roads that exist by the moment upgrading of the roads begun. Each of these lines contains three integers *x*~*i*~, *y*~*i*~, *l*~*i*~: *x*~*i*~, *y*~*i*~ — the numbers of the cities connected by this road (1 ≤ *x*~*i*~, *y*~*i*~ ≤ *n*, *x*~*i*~ ≠ *y*~*i*~), *l*~*i*~ — length of the road (1 ≤ *l*~*i*~ ≤ 10^9^). Note that one pair of cities can be connected with multiple roads.
