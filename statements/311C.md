...
- Add another method *x*: she can also go just *x* cells forward at any moment. For example, initially she has only one method *k*. If at some moment she has methods *a*~1~, *a*~2~, ..., *a*~*r*~ then she can reach all the cells with number in form

![](images/311C_1.png)

, where *v*~*i*~ — some non-negative integer.
- Reduce the value of the treasure in the *x*-th "Treasure Cell" by *y* dollars. In other words, to apply assignment *c*~*x*~ = *c*~*x*~ - *y*.
...
Each of the next *n* lines contains two integers: *a*~*i*~ (1 ≤ *a*~*i*~ ≤ *h*), *c*~*i*~ (1 ≤ *c*~*i*~ ≤ 10^9^). That means the *i*-th "Treasure Cell" is the *a*~*i*~-th cell and cost of the treasure in that cell is *c*~*i*~ dollars. All the *a*~*i*~ are distinct.

Each of the next *m* lines is in one of the three following formats:

- "1 *x*" — an operation of type 1, 1 ≤ *x* ≤ *h*;
- "2 *x* *y*" — an operation of type 2, 1 ≤ *x* ≤ *n*, 0 ≤ *y* < *c*~*x*~;
