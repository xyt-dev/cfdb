...
Each of the following *n* lines contains the time of trip *t*~*i*~ (0 ≤ *t*~*i*~ ≤ 10^9^), measured in minutes from the time of starting the system. All *t*~*i*~ are different, given in ascending order, i. e. *t*~*i* + 1~ > *t*~*i*~ holds for all 1 ≤ *i* < *n*.
