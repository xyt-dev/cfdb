...
It is guaranteed that the solution always exists and is unique. It is guaranteed that in the correct solution the coordinates of the polygon vertices are between 1 and *N* - 1. A vertex (*x*~1~, *y*~1~) is lexicographically smaller than vertex (*x*~2~, *y*~2~) if *x*~1~ < *x*~2~ or
