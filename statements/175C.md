...
Each of the following *n* lines contains two integer numbers *k*~*i*~ and *c*~*i*~ (1 ≤ *k*~*i*~ ≤ 10^9^, 0 ≤ *c*~*i*~ ≤ 1000), separated with space — the number of figures of the *i*-th type and the cost of one *i*-type figure, correspondingly.

The next line contains the only integer number *t* (1 ≤ *t* ≤ 100) — the number that describe the factor's changes.

The next line contains *t* integer numbers *p*~*i*~ (1 ≤ *p*~1~ < *p*~2~ < ... < *p*~*t*~ ≤ 10^12^), separated with spaces.
