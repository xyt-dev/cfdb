...
- Let's take any element *c* from this set. For all *j* (1 ≤ *j* ≤ *m*) if number (*c*·*a*~*i*~^*b*~*j*~^) *mod* *p* doesn't occur in the set, then add it to the set.
...
The second line contains space-separated integers *a*~1~, *a*~2~, ... , *a*~*n*~ (1 ≤ *a*~*i*~ < *p*). The third line contains space-separated integers *b*~1~, *b*~2~, ... , *b*~*m*~ (1 ≤ *b*~*i*~ ≤ 10^9^).
