...
) — the number of cities and the number of roads in Berland, correspondingly. Next *m* lines contain pairs of integers *v*~*i*~, *u*~*i*~ (1 ≤ *v*~*i*~, *u*~*i*~ ≤ *n*, *v*~*i*~ ≠ *u*~*i*~) — the numbers of cities that are connected by the *i*-th road. The numbers on a line are separated by a space.
