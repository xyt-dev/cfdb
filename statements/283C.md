...
The first line contains three space-separated integers, *n*, *q* and *t* (1 ≤ *n* ≤ 300; 0 ≤ *q* ≤ *n*; 1 ≤ *t* ≤ 10^5^). The second line contains *n* space separated integers, *a*~1~, *a*~2~, ..., *a*~*n*~ (1 ≤ *a*~*i*~ ≤ 10^5^). The next *q* lines each contain two distinct space-separated integers, *b*~*i*~ and *c*~*i*~ (1 ≤ *b*~*i*~, *c*~*i*~ ≤ *n*; *b*~*i*~ ≠ *c*~*i*~).
...
No other combinations exist. Note that even though 4 occurs in both *b*~*i*~ and *c*~*i*~,  the problem conditions are still satisfied because all *b*~*i*~ are distinct and all *c*~*i*~ are distinct.
