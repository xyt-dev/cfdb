...
Joi has an array $a$ of $n$ positive integers. Koxia wants you to determine whether there exists a positive integer $x \gt 0$ such that $\gcd(a_i+x,a_j+x)=1$ for all $1 \leq i \lt j \leq n$.
...
For each test case, output "YES" (without quotes) if there exists a positive integer $x$ such that $\gcd(a_i+x,a_j+x)=1$ for all $1 \leq i \lt j \leq n$, and "NO" (without quotes) otherwise.
