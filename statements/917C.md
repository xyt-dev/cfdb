...
The next line contains *k* integers, *c*~1~, *c*~2~, ... *c*~*k*~, separated by spaces (1 ≤ *c*~*i*~ ≤ 10^9^) — the energetic costs of jumps.

The next *q* lines contain description of the special stones. Each line contains two integers *p* and *w*~*p*~ (*x* + 1 ≤ *p* ≤ *n*, |*w*~*p*~| ≤ 10^9^). All *p* are distinct.
