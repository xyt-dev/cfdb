...
The first line contains two space-separated integers *n*, *m* (1 ≤ *n*, *m* ≤ 10^5^) — the number of students and the number of passengers the bus can transport, correspondingly. Next *n* lines contain descriptions of the students, one per line. Each line contains a pair of integers *t*~*i*~, *x*~*i*~ (1 ≤ *t*~*i*~ ≤ 10^5^, 1 ≤ *x*~*i*~ ≤ 10^4^). The lines are given in the order of strict increasing of *t*~*i*~. Values of *x*~*i*~ can coincide.
