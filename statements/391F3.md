...
The first line contains two integers *n* and *k*, separated by a single space, with

![](images/391F3_1.png)

. The *i*-th of the following *n* lines contains a single integer *p*~*i*~ (0 ≤ *p*~*i*~ ≤ 10^12^), where *p*~*i*~ represents the price at which someone can either buy or sell one share of stock on day *i*.
