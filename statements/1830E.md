...
- Let $j$ be the index of the smallest element $p_j$ such that $i \lt j$.
- Swap $p_i$ and $p_j$.
...
The $i$-th of the next $q$ lines of input contains two integers $x_i$ and $y_i$ ($1 \le x_i \lt y_i \le n$) — describing the $i$-th update.
