...
The next *n* - 1 lines each contain two distinct space-separated integers *u*~*i*~ and *v*~*i*~ (1 ≤ *u*~*i*~, *v*~*i*~ ≤ *n*) — the endpoints of the *i*-th bridge. Bridges form a tree, and it is guaranteed that no bridge is listed twice in the input.
