...
The first line contains integer *n* (2 ≤ *n* ≤ 10^5^) — the number of measurements Vasya made. The second line contains *n* integers *c*~1~, *c*~2~, ..., *c*~*n*~ (1 ≤ *c*~*i*~ ≤ 5000) — the results of the measurements. The numbers on the second line are separated by single spaces.
