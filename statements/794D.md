...
Otherwise, output the string "YES" (without quotes) on the first line. On the next line, output *n* space-separated integers, *x*~1~, *x*~2~, ..., *x*~*n*~. The condition 1 ≤ *x*~*i*~ ≤ 10^9^ must hold for all *i*, and for all pairs of cities (*u*, *v*) the condition |*x*~*u*~ - *x*~*v*~| ≤ 1 must hold if and only if there is a road connecting *u* and *v*.
