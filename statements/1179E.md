...
Each of $n$ following lines should contain $L$ numbers $l_1$, $l_2$, ... , $l_L$ ($0 \leq l_j \lt 10^{18}$ for all $1 \leq j \leq L$ and $l_j \lt l_{j+1}$ for all $1 \lt j \leq L$), in $i$-th of them $l_j$ means that $f_i(l_j) \lt f_i(l_j + 1)$.
