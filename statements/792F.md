...
*i*-th of each next *q* lines contains three numbers *k*~*i*~, *a*~*i*~ and *b*~*i*~ (1 ≤ *k*~*i*~ ≤ 2, 1 ≤ *a*~*i*~, *b*~*i*~ ≤ 10^6^).
...
- If *k*~*i*~ = 1, then character learns spell with *x* = (*a*~*i*~ + *j*) *mod* 10^6^ + 1, *y* = (*b*~*i*~ + *j*) *mod* 10^6^ + 1.
- If *k*~*i*~ = 2, then you have to determine if Vova is able to win the fight against monster with *t* = (*a*~*i*~ + *j*) *mod* 10^6^ + 1, *h* = (*b*~*i*~ + *j*) *mod* 10^6^ + 1.
