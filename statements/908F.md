...
The next *n* lines will contain two tokens *p*~*i*~ and *c*~*i*~ (*p*~*i*~ is an integer, 1 ≤ *p*~*i*~ ≤ 10^9^, *c*~*i*~ is a uppercase English letter 'R', 'G' or 'B'), denoting the position of the *i*-th point and the color of the *i*-th point. 'R' means red, 'G' denotes green, and 'B' means blue. The positions will be in strictly increasing order.
