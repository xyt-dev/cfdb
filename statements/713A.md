...
-  +  *a*~*i*~ — add non-negative integer *a*~*i*~ to the multiset. Note, that she has a multiset, thus there may be many occurrences of the same integer.
-  -  *a*~*i*~ — delete a single occurrence of non-negative integer *a*~*i*~ from the multiset. It's guaranteed, that there is at least one *a*~*i*~ in the multiset.
