...
- Select $1\le i \lt n$, decrease $a_i$ by $x\cdot t$, and decrease $a_{i+1}$ by $y\cdot t$.
- Select $1\le i \lt n$, decrease $a_i$ by $y\cdot t$, and decrease $a_{i+1}$ by $x\cdot t$.
...
- If it is $2$, it is followed by two integers $l$, $r$ ($1\le l \lt r\le n$). It means that you should print $f([b_l,b_{l+1},\dots,b_r])$.
