...
The following *n* lines contain requests, one request per line. The *i*-th request is given as the pair of integers *s*~*i*~, *d*~*i*~ (1 ≤ *s*~*i*~ ≤ 10^9^, 1 ≤ *d*~*i*~ ≤ 5·10^6^), where *s*~*i*~ is the preferred time to start repairing the *i*-th car, *d*~*i*~ is the number of days to repair the *i*-th car.
