...
Help Polycarpus, given matrix *b*, restore the sequence of numbers *a*~1~, *a*~2~, ..., *a*~*n*~, that he has removed from the board. Polycarpus doesn't like large numbers, so any number in the restored sequence mustn't exceed 10^9^.

#### Input

The first line contains a single integer *n* (1 ≤ *n* ≤ 100) — the size of square matrix *b*. Next *n* lines contain matrix *b*. The *i*-th of these lines contains *n* space-separated integers: the *j*-th number represents the element of matrix *b*~*ij*~. It is guaranteed, that for all *i* (1 ≤ *i* ≤ *n*) the following condition fulfills: *b*~*ii*~ = -1. It is guaranteed that for all *i*, *j* (1 ≤ *i*, *j* ≤ *n*; *i* ≠ *j*) the following condition fulfills: 0 ≤ *b*~*ij*~ ≤ 10^9^, *b*~*ij*~ = *b*~*ji*~.

#### Output

Print *n* non-negative integers *a*~1~, *a*~2~, ..., *a*~*n*~ (0 ≤ *a*~*i*~ ≤ 10^9^) — the sequence that Polycarpus wiped off the board. Separate the numbers by whitespaces.
