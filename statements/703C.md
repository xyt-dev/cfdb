...
The next *n* lines describes polygon vertices in counter-clockwise order. *i*-th of them contains pair of integers *x*~*i*~ and *y*~*i*~ ( - 10^9^ ≤ *x*~*i*~ ≤ 10^9^, 0 ≤ *y*~*i*~ ≤ *w*) — coordinates of *i*-th polygon point. It is guaranteed that the polygon is non-degenerate.
