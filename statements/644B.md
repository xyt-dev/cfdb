...
Then follow *n* lines with queries descriptions (in chronological order). Each description consists of two integers *t*~*i*~ and *d*~*i*~ (1 ≤ *t*~*i*~, *d*~*i*~ ≤ 10^9^), where *t*~*i*~ is the moment of time when the *i*-th query appears and *d*~*i*~ is the time server needs to process it. It is guaranteed that *t*~*i* - 1~ < *t*~*i*~ for all *i* > 1.
