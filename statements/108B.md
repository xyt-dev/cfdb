...
The first line contains integer *n* (2 ≤ *n* ≤ 10^5^) — the number of Gava's unsigned integer datatypes' sizes. The second line contains a single-space-separated list of *n* integers (1 ≤ *a*~*i*~ ≤ 10^9^) — sizes of datatypes in bits. Some datatypes may have equal sizes.
...
In the second example, *x* = 7 (111~2~) fits in 3 bits, but *x*^2^ = 49 (110001~2~) does not fit in 4 bits.
