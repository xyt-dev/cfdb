...
In one query, you can pick two integers $l$ and $r$ ($1 \le l \lt r \le n$) by paying $(r - l)^2$ coins. In return, you will be given the number of inversions$^\ddagger$ in the subarray $[p_l, p_{l + 1}, \ldots p_r]$.
...
$^\ddagger$ The number of inversions in an array is the number of pairs of indices $(i,j)$ such that $i \lt j$ and $a_i \gt a_j$. For example, the array $[10,2,6,3]$ contains $4$ inversions. The inversions are $(1,2),(1,3),(1,4)$, and $(3,4)$.
...
To make a query, output "? l r" ($1 \le l \lt r \le n$) without quotes. Afterwards, you should read one single integer — the answer for your query.
