...
- $l_1=1$, $r_m=n$;
- For all $1 \lt i\le m$, $l_i=r_{i-1}+1$.
...
In the second test case, if Daniel divides the array into $[2023]$ and $[2043]$, then for $i=1,j=2$ we have $2023+2043 \gt \max(2023,2043)$, so the maximum number of subsegments is $1$.
