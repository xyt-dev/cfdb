...
The first line of the input data contains an integer *n* (1 ≤ *n* ≤ 10^5^) which represents the number of registered clients of the "Couple Matching". The second line contains a sequence of integers *t*~1~, *t*~2~, ..., *t*~*n*~ ( - 10 ≤ *t*~*i*~ ≤ 10), *t*~*i*~ — is the parameter of the *i*-th customer that has been assigned to the customer by the result of the analysis of his interests.
