...
- Otherwise,
 ...
 - If $a \gt v$, the probability of the item drawn reduces by $v$ and the reduced probability is distributed equally among the other valid items.
...
The first and the only line of each test case contains four real numbers $c$, $m$, $p$ and $v$ ($0 \lt c,m,p \lt 1$, $c+m+p=1$, $0.1\leq v\leq 0.9$).
