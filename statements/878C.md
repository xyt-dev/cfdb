...
Each of the next *n* lines contains *k* integers *s*~*i*1~, *s*~*i*2~, ..., *s*~*ik*~ (1 ≤ *s*~*ij*~ ≤ 10^9^), where *s*~*ij*~ is the power of the *i*-th sportsman in the *j*-th kind of sport. The sportsman with higher powers always wins. It's guaranteed that for any kind of sport all of these powers are distinct.
