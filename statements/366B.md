...
The first line of the input contains two integers *n*, *k* (1 ≤ *k* ≤ *n* ≤ 10^5^). The second line contains *n* integers *a*~1~, *a*~2~, ..., *a*~*n*~ (1 ≤ *a*~*i*~ ≤ 10^3^), where *a*~*i*~ is the power Inna tells Dima off with if she is present in the room while he is doing the *i*-th task.
