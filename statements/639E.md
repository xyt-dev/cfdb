...
Points given for solving a problem decrease linearly. Solving the *i*-th problem after *x* minutes gives exactly
...
![](images/639E_2.png)
 ...
 is some real constant that Limak must choose.
...
It's not hard to prove that there will be no paradox for *c* = 0. The situation may be worse for bigger *c*. What is the maximum real value *c* (remember that

![](images/639E_3.png)

) for which there is no paradox possible, that is, there will be no paradox for any optimal order of solving problems?
...
The second line contains *n* integers *p*~1~, *p*~2~, ..., *p*~*n*~ (1 ≤ *p*~*i*~ ≤ 10^8^) — initial scores.

The third line contains *n* integers *t*~1~, *t*~2~, ..., *t*~*n*~ (1 ≤ *t*~*i*~ ≤ 10^8^) where *t*~*i*~ is the number of minutes needed to solve the *i*-th problem.

#### Output

Print one real value on a single line — the maximum value of *c* that

![](images/639E_4.png)

 and there is no optimal order with a paradox. Your answer will be considered correct if its absolute or relative error does not exceed 10^ - 6^.
