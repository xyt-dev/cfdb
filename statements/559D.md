...
Next *n* lines contain the coordinates of the vertices of the basic polygon. The *i*-th of these lines contain two integers *x*~*i*~ and *y*~*i*~ ( - 10^9^ ≤ *x*~*i*~, *y*~*i*~ ≤ 10^9^) — the coordinates of the *i*-th vertex of the polygon. The vertices are given in the counter-clockwise order.
...
Let's assume that a random variable takes values *x*~1~, ..., *x*~*n*~ with probabilities *p*~1~, ..., *p*~*n*~, correspondingly. Then the expected value of this variable equals to
