...
Then, you have to choose an integer $k$ ($1 \le k \lt m$) and cut the colored matrix in such a way that the first $k$ columns become a separate matrix (the left matrix) and the last $m-k$ columns become a separate matrix (the right matrix).
