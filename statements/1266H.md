...
Then $q$ lines with queries follow. The $j$-th of these lines contains an integer $v$ ($1 \leq v \lt n$) and a string $s$ of length $n-1$ consiting only of characters 'R' and 'B'. The $i$-th of these characters is 'R' if the red arc going from $i$ is active and 'B' otherwise.
