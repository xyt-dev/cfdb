...
- For all $1 \le i \lt n - 1$, $\gcd(a_i, a_{i + 1})$ is a prime number.
...
The second line of each test case contains $m$ integers $b_1, b_2, \ldots, b_m$ ($1 \le b_i \lt 2^{20}$) — exponents of corresponding prime factors of $c$, so that $c = p_1^{b_1} \cdot p_2^{b_2} \cdot \ldots \cdot p_m^{b_m}$ and $n = (b_1 + 1)(b_2 + 1) \ldots (b_m + 1)$ hold. $p_i$ is the $i$-th smallest prime number.
