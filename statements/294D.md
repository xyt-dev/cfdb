...
The first line of the input contains two integers *n* and *m*, (2 ≤ *n*, *m* ≤ 10^5^). The second line contains two integers *x*~*s*~ and *y*~*s*~ (1 ≤ *x*~*s*~ ≤ *n*, 1 ≤ *y*~*s*~ ≤ *m*) and the direction robot is facing initially. Direction is one of the strings: "UL" (upper-left direction), "UR" (upper-right), "DL" (down-left) or "DR" (down-right).
