...
The following *m* lines provide the description of the edges, as the *i*-th of them contains two integers *u*~*i*~ and *v*~*i*~ (1 ≤ *u*~*i*~, *v*~*i*~ ≤ *n*, *u*~*i*~ ≠ *v*~*i*~) — the indices of the vertices connected by the *i*-th edge, and a character *c*~*i*~ (

![](images/662B_1.png)

) providing the initial color of this edge. If *c*~*i*~ equals 'R', then this edge is initially colored red. Otherwise, *c*~*i*~ is equal to 'B' and this edge is initially colored blue. It's guaranteed that there are no self-loops and multiple edges.
