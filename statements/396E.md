...
Each of the next *m* lines contains a pair of space-separated integers *p*~*i*~, *a*~*i*~ (2 ≤ *p*~*i*~ ≤ 10^6^; 1 ≤ *a*~*i*~ ≤ 10^17^) — another prime divisor of number *n* and its power in the canonical representation. The sum of all *a*~*i*~ doesn't exceed 10^17^. Prime divisors in the input follow in the strictly increasing order.
