...
Peaks of mountains on the segment $[l,r]$ (from $l$ to $r$) are called indexes $i$ such that $l \lt i \lt r$, $a_{i - 1} \lt a_i$ and $a_i \gt a_{i + 1}$. It is worth noting that the boundary indexes $l$ and $r$ for the segment are not peaks. For example, if $n=8$ and $a=[3,1,4,1,5,9,2,6]$, then the segment $[1,8]$ has only two peaks (with indexes $3$ and $6$), and there are no peaks on the segment $[3, 6]$.
