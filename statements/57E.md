...
The first line contains two integers *k* and *n* (0 ≤ *k* ≤ 10^18^, 0 ≤ *n* ≤ 440) which are correspondingly the maximal number of moves a knight can make and the number of deleted cells. Then follow *n* lines, each giving the coordinates of a deleted square in the form (*x*~*i*~, *y*~*i*~) (|*x*~*i*~| ≤ 10, |*y*~*i*~| ≤ 10). All the numbers are integer, the deleted squares are different and it is guaranteed that the square (0, 0) is not deleted.
