...
- Given two indices, $l$ and $r$ ($1 \le l \lt r \le n$). It's guaranteed that the $l$-th character is equal to "(", the $r$-th character is equal to ")", and the characters between them are equal to ".". Then the $l$-th and the $r$-th characters must be set to ".".
- Given two indices, $l$ and $r$ ($1 \le l \lt r \le n$), and it's guaranteed that the substring $s[l\dots r]$ is a simple RBS. You need to find the number of substrings in $s[l\dots r]$ such that they are simple RBS. In other words, find the number of index pairs $i$, $j$ such that $l \le i \lt j \le r$ and $s[i\dots j]$ is a simple RBS.
...
Each of the following $q$ lines contains three integers $t$, $l$ and $r$ ($t \in \{1, 2\}$, $1 \le l \lt r \le n$), the queries you need to answer. It is guaranteed that all the queries are valid and correspond to the problem statements.
