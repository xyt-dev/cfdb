...
The first line of the input contains a single number *n* (1 ≤ *n* ≤ 10^5^) — the number of shelves at Winnie's place. The second line contains *n* integers *a*~*i*~ (1 ≤ *i* ≤ *n*, 0 ≤ *a*~*i*~ ≤ 100) — the number of honey pots on a shelf number *i*.
