...
The following *m* lines contain information about ride requests. Each ride request is represented by integers *t*~*j*~, *a*~*j*~ and *b*~*j*~ (1 ≤ *t*~*j*~ ≤ 10^12^, 1 ≤ *a*~*j*~, *b*~*j*~ ≤ *n*, *a*~*j*~ ≠ *b*~*j*~), where *t*~*j*~ is time in minutes when a request is made, *a*~*j*~ is a house where passenger needs to be picked up, and *b*~*j*~ is a house where passenger needs to be dropped off. All taxi ride requests are given in the increasing order of *t*~*j*~. All *t*~*j*~ are distinct.
