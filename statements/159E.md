...
The first line contains an integer *n* (2 ≤ *n* ≤ 10^5^) — the number of cubes. Next *n* lines contain the descriptions of the cubes, one description per line. A cube description consists of two space-separated integers *c*~*i*~ and *s*~*i*~ (1 ≤ *c*~*i*~, *s*~*i*~ ≤ 10^9^) — the *i*-th cube's color and size, correspondingly. It is guaranteed that there are at least two cubes of different colors.
