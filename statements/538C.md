...
Next *m* lines contain two space-separated integers *d*~*i*~ and *h*~*d*~*i*~~ (1 ≤ *d*~*i*~ ≤ *n*, 0 ≤ *h*~*d*~*i*~~ ≤ 10^8^) — the number of the day when the *i*-th note was made and height on the *d*~*i*~-th day. It is guaranteed that the notes are given in the chronological order, i.e. for all *i* from 1 to *m* - 1 the following condition holds: *d*~*i*~ < *d*~*i* + 1~.
