...
- add *v* *e*Petya adds *e* electrons to vertex number *v* (1 ≤ *v* ≤ 2^*h* + 1^ - 1, 0 ≤ *e* ≤ 10^4^). *v* and *e* are integers.The vertices of the tree are numbered in the following way: the root is numbered with 1, the children of the vertex with number *x* are numbered with 2*x* and 2*x* + 1.
