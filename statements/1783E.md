...
Let $k = 1$. First, Monocarp makes one attempt to kill the first boss. It's successful, since $a_1 = 1$. Then, Monocarp makes one attempt to kill the second boss. It's unsuccessful, since $a_2 \gt 1$. So, Polycarp makes an attempt then. It's also unsuccessful, since $b_2 \gt 1$. Then, Monocarp makes another attempt. It's still unsuccessful, since $a_2 \gt 2$. This goes on until Polycarp finally kills the boss on his third attempt. Monocarp didn't kill this boss, thus, $k = 1$ isn't the answer.
