...
A path is a sequence of mountains $M$ such that for each $i$ ($1 \le i \lt |M|$), there exists a road between $M_i$ and $M_{i+1}$. Boboniu would regard the path as a challenge if for each $i$ ($1\le i \lt |M|$), $h_{M_i}\le h_{M_{i+1}}$.
