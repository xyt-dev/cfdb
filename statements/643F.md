...
![](images/643F_3.png)
 ...
 denotes the exclusive or (also denoted as XOR).

Note that the same barrel may be chosen by many bears and all of them will go to sleep at once.

#### Input

The only line of the input contains three integers *n*, *p* and *q* (1 ≤ *n* ≤ 10^9^, 1 ≤ *p* ≤ 130, 1 ≤ *q* ≤ 2 000 000) — the number of bears, the number of places to sleep and the number of scenarios, respectively.
...
In the first sample, there are 5 bears and only 1 place to sleep. We have *R*~1~ = 6, *R*~2~ = 11, *R*~3~ = 16 so the answer is

![](images/643F_5.png)

. Let's analyze the optimal strategy for scenario with 2 days. There are *R*~2~ = 11 barrels and 10 of them contain juice.
...
In the second sample, there is only one bear. He should choose an empty set of barrels in each night. Otherwise, he would maybe get wine and bears would lose (because there must be at least one awake bear). So, for any number of days we have *R*~*i*~ = 1. The answer is
