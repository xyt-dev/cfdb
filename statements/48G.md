...
The first line contains an integer *n* (3 ≤ *n* ≤ 200000) which represents the number of planets in the Galaxy and the number of communication channels equal to it. The next *n* lines contain three integers each *a*~*i*~, *b*~*i*~ and *t*~*i*~ (1 ≤ *a*~*i*~, *b*~*i*~ ≤ *n*, *a*~*i*~ ≠ *b*~*i*~, 1 ≤ *t*~*i*~ ≤ 10^3^) that represent the numbers of planet joined by a communication channel and its "dial duration". There can be no more than one communication channel between a pair of planets.
