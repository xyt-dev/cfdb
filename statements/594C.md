...
Let us explain the last two sentences. Let's suppose we want to hang two magnets on the refrigerator. If the magnet in the plan has coordinates of the lower left corner (*x*~1~, *y*~1~) and the upper right corner (*x*~2~, *y*~2~), then its center is located at (
...
Next *n* lines describe the initial plan of placing magnets. Each line contains four integers *x*~1~, *y*~1~, *x*~2~, *y*~2~ (1 ≤ *x*~1~ < *x*~2~ ≤ 10^9^, 1 ≤ *y*~1~ < *y*~2~ ≤ 10^9^) — the coordinates of the lower left and upper right corners of the current magnet. The magnets can partially overlap or even fully coincide.
