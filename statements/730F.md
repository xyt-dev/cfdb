...
Initially, there are *b* bonuses on Polycarp's account. Polycarp is going to have a lunch in "Ber Patio" for the next *n* days. He estimated the values *a*~1~, *a*~2~, ..., *a*~*n*~, where *a*~*i*~ is the number of burles in a receipt for the *i*-th day. The sum over all receipts doesn't exceed 10^5^ burles.
