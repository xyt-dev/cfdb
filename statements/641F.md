...
. Note that there might be negations in 2-SAT formula (like for *x*~1~ and for *x*~4~).
...
If both formulas share the same set of solutions, output a single word "SIMILAR" (without quotes). Otherwise output exactly *n* integers *x*~*i*~ (
...
In second sample if we compute first function with *x*~1~ = 0 and *x*~2~ = 0 we get the result 0, because
