...
The first line contains two integers *n*, *m* (1 ≤ *n*, *m* ≤ 10^5^). The next *n* lines contain pairs of integers. The *i*-th line contains integers *c*~*i*~, *t*~*i*~ (1 ≤ *c*~*i*~, *t*~*i*~ ≤ 10^9^) — the description of the play list. It is guaranteed that the play list's total duration doesn't exceed 10^9^
