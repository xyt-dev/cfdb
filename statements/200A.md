...
The first input line contains three integers *n*, *m*, *k* (1 ≤ *n*, *m* ≤ 2000, 1 ≤ *k* ≤ *min*(*n*·*m*, 10^5^) — the number of rows in the room, the number of seats in each row and the number of people in the line, correspondingly. Each of the next *k* lines contains two integers *x*~*i*~, *y*~*i*~ (1 ≤ *x*~*i*~ ≤ *n*, 1 ≤ *y*~*i*~ ≤ *m*) — the coordinates of the seat each person has chosen. Numbers on the same line are separated by a space. The pairs of coordinates are located in the order, in which people stand in the line, starting from the head (the first person in the line who stands in front of the box office) to the tail (the last person in the line).
