...
The first line contains the number *n* (3 ≤ *n* ≤ 10^4^) — the amount of vertices of the fence. Then there follow *n* lines containing the coordinates of these vertices (two integer numbers *x*~*i*~, *y*~*i*~) in clockwise or counter-clockwise order. It's guaranteed, that the fence does not contain self-intersections.
