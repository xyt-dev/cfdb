...
Following this is a line with 2·*N* - 1 integers, *s*~1~ through *s*~2·*N* - 1~ (0 ≤ *s*~*i*~ ≤ 10^8^). For odd *i*, *s*~*i*~ indicates the number of contestants desiring T-shirt size ((*i* + 1) / 2). For even *i*, *s*~*i*~ indicates the number of contestants okay receiving either of T-shirt sizes (*i* / 2) and (*i* / 2 + 1). *C* will not exceed the total number of contestants.
