...
The first line contains integer *n* (2 ≤ *n* ≤ 10^5^), showing the number of vertices in the tree. The next line contains *n* integers *a*~1~, *a*~2~, ..., *a*~*n*~ (0 ≤ *a*~*i*~ ≤ 10^8^), *a*~*i*~ is the number of apples in the vertex number *i*. The number of apples in non-leaf vertices is guaranteed to be zero.
