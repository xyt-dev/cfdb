...
In one move, you can move one block from tower $i$ to tower $j$, but only if $a_i \gt a_j$. That move increases $a_j$ by $1$ and decreases $a_i$ by $1$. You can perform as many moves as you would like (possibly, zero).
