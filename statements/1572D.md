...
There are currently $n$ hot topics numbered from $0$ to $n-1$ at your local bridge club and $2^n$ players numbered from $0$ to $2^n-1$. Each player holds a different set of views on those $n$ topics, more specifically, the $i$-th player holds a positive view on the $j$-th topic if $i\ \&\ 2^j \gt 0$, and a negative view otherwise. Here $\&$ denotes the [bitwise AND operation](<https://en.wikipedia.org/wiki/Bitwise_operation#AND>).
