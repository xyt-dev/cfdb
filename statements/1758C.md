...
$^{\ddagger}$ Let $a$ and $b$ be permutations of length $n$. Then $a$ is lexicographically smaller than $b$ if in the first position $i$ where $a$ and $b$ differ, $a_i \lt b_i$. A permutation is lexicographically minimal if it is lexicographically smaller than all other permutations.
...
The only line of each test case contains two integers $n$ and $x$ ($2 \leq n \leq 2 \cdot 10^5$; $1 \lt x \leq n$).
