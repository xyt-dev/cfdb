...
The next $n-1$ lines of each test case each contains three integers, $x$, $y$, and $v$ ($1 \leq x, y \leq n$; $-1 \leq v \lt 2^{30}$) — meaning that there's an edge between nodes $x$ and $y$. If
...
If the answer is YES, print $n-1$ lines each containing three integers: $x$, $y$, and $v$ ($1 \le x, y \le n$; $0 \le v \lt 2^{30}$) — the edge and the integer on that edge. The set of edges must be the same as in the input, and if the value of some edge was specified earlier, it can not change. You can print the edges in any order.
