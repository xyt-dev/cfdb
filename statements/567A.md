...
The first line of the input contains integer *n* (2 ≤ *n* ≤ 10^5^) — the number of cities in Lineland. The second line contains the sequence of *n* distinct integers *x*~1~, *x*~2~, ..., *x*~*n*~ ( - 10^9^ ≤ *x*~*i*~ ≤ 10^9^), where *x*~*i*~ is the *x*-coordinate of the *i*-th city. All the *x*~*i*~'s are distinct and follow in ascending order.
