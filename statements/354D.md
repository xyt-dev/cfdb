...
- Change the value of some subpyramid. The picture shows a highlighted subpyramid with the top in cell 5. It is described by *s* + 2 numbers: "*t* *i* *v*~1~ *v*~2~ ... *v*~*s*~", where *t* = 2, *i* — the number of the top cell of the pyramid, *s* — the size of the subpyramid (the number of cells it has), *v*~*j*~ — the value you should assign to the *j*-th cell of the subpyramid.
