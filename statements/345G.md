...
The first line contains an integer *n* (1 ≤ *n* ≤ 10^5^) — the number of strings in the group. Each of the next *n* lines contains a string. The *i*-th line contains non-empty string *s*~*i*~.

Each string consists only from lowercase Latin letters. The sum of all strings *s*~*i*~ doesn't exceed 10^5^.
