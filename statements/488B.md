...
There is an old tradition of keeping 4 boxes of candies in the house in Cyberland. The numbers of candies are special if their arithmetic mean, their median and their range are all equal. By definition, for a set {*x*~1~, *x*~2~, *x*~3~, *x*~4~} (*x*~1~ ≤ *x*~2~ ≤ *x*~3~ ≤ *x*~4~) arithmetic mean is
...
![](images/488B_2.png)
 ...
 and range is *x*~4~ - *x*~1~. The arithmetic mean and median are not necessary integer. It is well-known that if those three numbers are same, boxes will create a "debugging field" and codes in the field will have no bugs.
