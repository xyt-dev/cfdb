...
The second line contains *n* integers *a*~1~, *a*~2~, ..., *a*~*n*~ (1 ≤ *a*~*i*~ ≤ 10^9^) — the integers written in the vertices.

The next (*n* - 1) lines contain two integers each. The *i*-th of these lines contains integers *p*~*i*~ and *w*~*i*~ (1 ≤ *p*~*i*~ ≤ *n*, 1 ≤ *w*~*i*~ ≤ 10^9^) — the parent of the (*i* + 1)-th vertex in the tree and the number written on the edge between *p*~*i*~ and (*i* + 1).
