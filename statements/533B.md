...
Then *n* lines follow, describing the company employees. The *i*-th line contains two integers *p*~*i*~, *a*~*i*~ (1 ≤ *a*~*i*~ ≤ 10^5^) — the number of the person who is the *i*-th employee's immediate superior and *i*-th employee's efficiency. For the director *p*~1~ =  - 1, for all other people the condition 1 ≤ *p*~*i*~ < *i* is fulfilled.
