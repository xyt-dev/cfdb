...
- Type B: Replace $x$ with the floor of half its value. Formally, update $x := \left\lfloor\frac{x}{2}\right\rfloor$, where $\lfloor y\rfloor$ denotes the [floor](<https://en.wikipedia.org/wiki/Floor_and_ceiling_functions>) of $y$, which is the greatest integer less than or equal to $y$.
