...
The following *n* lines each describes a dancer: the *i*-th line among them contains three space-separated integers *x*~*i*~, *y*~*i*~ and *r*~*i*~ ( - 10^6^ ≤ *x*~*i*~, *y*~*i*~ ≤ 10^6^, 1 ≤ *r*~*i*~ ≤ 10^6^), describing a circular movement range centered at (*x*~*i*~, *y*~*i*~) with radius *r*~*i*~.
