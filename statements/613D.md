...
Each of the next *n* - 1 lines contains two distinct integers *u*~*i*~, *v*~*i*~ (1 ≤ *u*~*i*~, *v*~*i*~ ≤ *n*) — the indices of the cities connected by the *i*-th road. It is guaranteed that you can get from any city to any other one moving only along the existing roads.

The next line contains a single integer *q* (1 ≤ *q* ≤ 100 000) — the number of King's plans.

Each of the next *q* lines looks as follows: first goes number *k*~*i*~ — the number of important cities in the King's plan, (1 ≤ *k*~*i*~ ≤ *n*), then follow exactly *k*~*i*~ space-separated pairwise distinct numbers from 1 to *n* — the numbers of important cities in this plan.
