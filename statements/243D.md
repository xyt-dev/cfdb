...
The first line contains three integers *n*, *v*~*x*~ and *v*~*y*~ (1 ≤ *n* ≤ 10^3^, |*v*~*x*~|, |*v*~*y*~| ≤ |10^4^|, |*v*~*x*~| + |*v*~*y*~| > 0).

Next *n* lines contain *n* integers each: the *j*-th integer in the *i*-th line *a*~*ij*~ (0 ≤ *a*~*ij*~ ≤ 10^9^, 1 ≤ *i*, *j* ≤ *n*) represents the height of the cube tower that stands on the unit square with opposite corners at points (*i* - 1, *j* - 1) and (*i*, *j*).
