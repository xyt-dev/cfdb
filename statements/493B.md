...
The following *n* lines contain integer numbers *a*~*i*~ (|*a*~*i*~| ≤ 10^9^, *a*~*i*~ ≠ 0). If *a*~*i*~ is positive, that means that the first wrestler performed the technique that was awarded with *a*~*i*~ points. And if *a*~*i*~ is negative, that means that the second wrestler performed the technique that was awarded with ( - *a*~*i*~) points.
