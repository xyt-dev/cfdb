...
Next *n* lines contain the descriptions of the segments. The *i*-th line contains two positive integers *l*~*i*~, *r*~*i*~ (1 ≤ *l*~*i*~ ≤ *r*~*i*~ ≤ 10^6^) — the borders of the *i*-th segment.

Next *m* lines contain the description of the queries, one per line. Each line starts from integer *cnt*~*i*~ (1 ≤ *cnt*~*i*~ ≤ 3·10^5^) — the number of points in the *i*-th query. Then the line contains *cnt*~*i*~ distinct positive integers *p*~1~, *p*~2~, ..., *p*~*cnt*~*i*~~ (1 ≤ *p*~1~ < *p*~2~ < ... < *p*~*cnt*~*i*~~ ≤ 10^6^) — the coordinates of points in the *i*-th query.
