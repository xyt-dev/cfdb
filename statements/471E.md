...
The first line of the input contains integer *n* (1 ≤ *n* ≤ 2·10^5^) — the number of segments on the draft. The next *n* lines contain four integers each: *x*~1~, *y*~1~, *x*~2~, *y*~2~ ( - 10^9^ ≤ *x*~1~ ≤ *x*~2~ ≤ 10^9^;  - 10^9^ ≤ *y*~1~ ≤ *y*~2~ ≤ 10^9^) — the two startpoint and the two endpoint coordinates of a segment. All segments are non-degenerative and either are strictly horizontal or strictly vertical.
