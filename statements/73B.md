...
The first line contains number *n* (1 ≤ *n* ≤ 10^5^) — number of racers. Each of the next *n* lines contains *s*~*i*~ and *a*~*i*~ — nick of the racer (nonempty string, which consist of no more than 20 lowercase Latin letters) and the racer's points (0 ≤ *a*~*i*~ ≤ 10^6^). Racers are given in the arbitrary order.

The next line contains the number *m* (0 ≤ *m* ≤ *n*). Then *m* nonnegative integer numbers *b*~*i*~ follow. *i*-th number is equal to amount of points for the *i*-th awarded place (0 ≤ *b*~*i*~ ≤ 10^6^).
