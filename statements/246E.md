...
The first line of the input contains a single integer *n* (1 ≤ *n* ≤ 10^5^) — the number of people in the tree. Next *n* lines contain the description of people in the tree. The *i*-th line contains space-separated string *s*~*i*~ and integer *r*~*i*~ (0 ≤ *r*~*i*~ ≤ *n*), where *s*~*i*~ is the name of the man with a number *i*, and *r*~*i*~ is either the number of the direct ancestor of the man with a number *i* or 0, if the man with a number *i* has no direct ancestor.

The next line contains a single integer *m* (1 ≤ *m* ≤ 10^5^) — the number of Polycarpus's records. Next *m* lines contain space-separated pairs of integers. The *i*-th line contains integers *v*~*i*~, *k*~*i*~ (1 ≤ *v*~*i*~, *k*~*i*~ ≤ *n*).
