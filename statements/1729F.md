...
You are given $n$, $s$ and an integer $w$ ($1 \le w \lt n$).
...
The second line contains two integers $w, m$ ($1 \le w \lt n, 1 \le m \le 2 \cdot 10^5$), where $n$ — is the length of the given string $s$. The number $w$ denotes the lengths of the substrings being searched for, and $m$ is the number of queries to be processed.
