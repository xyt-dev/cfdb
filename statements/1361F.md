...
The instruction defines subsegment $[L, R]$ as good if $W_{L - 1} \lt W_i$ and $W_R \lt W_i$ for any $i$ in $\{L, L + 1, \ldots, R - 1\}$. For such subsegment it also defines $W_M$ as minimum of set $\{W_L, W_{L + 1}, \ldots, W_{R - 1}\}$.
