...
In the first line of the input the four integers *x*~0~, *y*~0~, *v*, *T* (|*x*~0~|, |*y*~0~| ≤ 10^9^, 0 ≤ *v*, *T* ≤ 10^9^) are given — the cockroach initial position on the table in the Cartesian system at the moment 0, the cockroach's constant speed and the time in seconds Dima needs for aiming respectively.

In the next line the only number *n* (1 ≤ *n* ≤ 100 000) is given — the number of shadow circles casted by plates.

In the next *n* lines shadow circle description is given: the *i*^*th*^ of them consists of three integers *x*~*i*~, *y*~*i*~, *r*~*i*~ (|*x*~*i*~|, |*y*~*i*~| ≤ 10^9^, 0 ≤ *r* ≤ 10^9^) — the *i*^*th*^ shadow circle on-table position in the Cartesian system and its radius respectively.
