...
The first line contains integer *n* (1 ≤ *n* ≤ 10^5^) — the number of nodes in the tree. The next *n* - 1 lines contain the tree edges. The *i*-th line contains integers *a*~*i*~, *b*~*i*~ (1 ≤ *a*~*i*~, *b*~*i*~ ≤ *n*; *a*~*i*~ ≠ *b*~*i*~) — the numbers of the nodes that are connected by the *i*-th edge.
