...
The input consists of a single line containing four integers *l*~1~, *r*~1~, *l*~2~, *r*~2~ (1 ≤ *l*~*i*~ ≤ *r*~*i*~ ≤ 10^9^, *i* = 1, 2). The numbers are separated by single spaces. *l*~*i*~ and *r*~*i*~ give the indices of the first and the last characters of the *i*-th chosen substring, correspondingly (*i* = 1, 2). The characters of string abracadabra are numbered starting from 1.
