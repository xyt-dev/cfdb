...
- the greatest common divisor of any two elements that are colored the same color is greater than $1$, i.e. $\gcd(a_i, a_j) \gt 1$ for each pair $i, j$ if these elements are colored the same color.
