...
For each index $i$, you are given $s_{i}$, which equals to the sum of all $p_{j}$ such that $j \lt i$ and $p_{j} \lt p_{i}$. In other words, $s_i$ is the sum of elements before the $i$-th element that are smaller than the $i$-th element.
