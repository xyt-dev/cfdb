...
The first line contains integer *n* (1 ≤ *n* ≤ 3·10^5^) — the number of vertices in the tree. The second line contains *n* - 1 integers *p*~2~, *p*~3~, ... *p*~*n*~ (1 ≤ *p*~*i*~ < *i*), where *p*~*i*~ is the number of the vertex that is the parent of vertex *i* in the tree.
