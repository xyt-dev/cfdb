...
Each of the next *m* lines contains a pair of integers *u*~*i*~, *v*~*i*~ (1 ≤ *u*~*i*~, *v*~*i*~ ≤ *n*) — a one-way road from the city *u*~*i*~ to the city *v*~*i*~. Note that *u*~*i*~ and *v*~*i*~ are not required to be distinct. Moreover, it can be several one-way roads between the same pair of cities.
