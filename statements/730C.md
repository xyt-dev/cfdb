...
The next line contains a single integer *w* (1 ≤ *w* ≤ 5000) — the total number of Bulmart stores in Berland. Each of the next *w* lines contains three integers describing the *i*-th store: *c*~*i*~, *k*~*i*~, *p*~*i*~ (1 ≤ *c*~*i*~ ≤ *n*, 1 ≤ *k*~*i*~, *p*~*i*~ ≤ 2·10^5^).

The next line contains a single integer *q* (1 ≤ *q* ≤ 1000) — the number of queries. Each of the next *q* lines contains three integers describing the *j*-th query: *g*~*j*~, *r*~*j*~ and *a*~*j*~ (1 ≤ *g*~*j*~ ≤ *n*, 1 ≤ *r*~*j*~, *a*~*j*~ ≤ 10^9^)
