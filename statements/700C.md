...
Then follow *m* lines, each of them contains three integers *x*~*i*~, *y*~*i*~ and *w*~*i*~ (1 ≤ *x*~*i*~, *y*~*i*~ ≤ *n*, 1 ≤ *w*~*i*~ ≤ 10^9^) — indices of towns connected by the *i*-th road, and the budget on its closure.
