...
The first input line contains an integer *n* which is the number of people who has joined the queue (1 ≤ *n* ≤ 10^5^). In the next *n* lines descriptions of the people are given in order of their coming — space-separated integers *a*~*i*~ and *c*~*i*~ (1 ≤ *a*~*i*~ ≤ *n*, 0 ≤ *c*~*i*~ ≤ *n*). Every description is located on s single line. All the *a*~*i*~'s are different.
