...
**Prepared by [Asymmetry](<https://codeforces.com/profile/Asymmetry>)**
 ...
 **Prepared by [Markadiusz](<https://codeforces.com/profile/Markadiusz>)**
## 1572B - Xor of 3
...
In the case of even $n$ we will find a prefix of odd length and even xor of numbers and call the above solution on it and its respective suffix. If there is no such prefix, the solution doesn't exist. Here is a proof of it. Assume that every prefix of odd length has an odd xor. This means that $a_1 = a_n = 1$ and for every even $i \lt n$, $a_i = a_{i+1}$. Consider an operation on an even position $i$ (odd is analogous). We know that $a_i = a_{i+1}$ so after this operation $a_i$ and $a_{i+1}$ will be set to $a_{i+2}$. This means that after every operation for every even $i \lt n$, $a_i = a_{i+1}$ still holds. Thus we will never be able to make $a_1$ equal to $0$ since performing an operation on it won't change it.
...
Let $dp[i][j]$ be the maximum number of operations that we can avoid on the interval from $i$ to $j$, while making all of its elements have the same color. Then the answer to the problem will be $n-1-dp[1][n]$. For $i \ge j$ we have $dp[i][j] = 0$ and for $i \lt j$ $dp[i][j]$ will be the maximum of $dp[i + 1][j]$ and $\max(1 + dp[i + 1][k - 1] + dp[k][j])$ over $i \lt k\le j$ such that $a[i]=a[k]$. It's because we can either not save any operations on the $i$-th element and just take the answer from the interval $[i + 1, j]$ or we can save one operation while coloring the segment from $i$ to $k$ and take the answer from segments $[i+1, k-1]$ and $[k, j]$. Because each color occurs in $a$ at most $20$ times, we can calculate this $dp$ in $O(20n^2)$ which is also our final time complexity.
 **Prepared by [Markadiusz](<https://codeforces.com/profile/Markadiusz>)**
## 1572D - Bridge Club
