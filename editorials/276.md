...
Let’s create some array *d*. When we have query *l*~*i*~, *r*~*i*~, we should increase value *d*[*l*~*i*~] by 1 and decrease value *d*[*r*~*i*~ + 1] by 1. In such a tricky way we increase all elements in segment [*l*~*i*~;*r*~*i*~] by 1 After processing all of the queries we need to make a loop, which visit every element of array *d*. In this loop we can easily calculate all elements of array *b*.
