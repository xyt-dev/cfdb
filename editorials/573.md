...
It's also possible to write it by adding "if" in right place in brute forces to get *O*(*n*^2^ + *nm*). Check it out in [code](<https://codeforces.com/contest/574/submission/12767226>).
...
After *k* operations we get *h*~*i*~ = *min*(*Left*, *Right*) where *Left* = *min*(*h*~*i* - *j*~ - (*k* - *j*)) = *min*(*h*~*i* - *j*~ + *j* - *k*) for

![](eimages/573_1.png)

 and *Right* is defined similarly. *h*~*i*~ becomes zero when *Left* or *Right* becomes zero. And *Left* becomes zero when *k* = *min*(*h*~*i* - *j*~ + *j*) — we will find this value for all *i*. If you are now lost in this editorial, try to draw some test and analyze my formulas with it.
...
![](eimages/573_10.png)
 ...
 linear functions of form *a*~*i*~·*x* + *b*. With binary search we can find the best element in
...
. And we can sort linear functions *a*~*i*~·*x* + *b* by angle only once because value *a*~*i*~ doesn't change — then constructing a hull is only
