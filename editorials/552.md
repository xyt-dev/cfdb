...
1) With every new rectangle we will add his area to the result, so for each line *x*~1~, *y*~1~, *x*~2~, *y*~2~ we will add to answer (*x*~2~ - *x*~1~ + 1) * (*y*~2~ - *y*~1~ + 1)
...
We also could just try 10 options *n* < 10, *n* < 100, ..., *n* < 10^9^, *n* = 10^9^ and solve problem for each option.
