...
Trivial solution will just emulate the work of all designers, every time considering all characters of the string one by one and replacing all *x*~*i*~ with *y*~*i*~ and vice versa. This will work in *O*(*n*·*m*) and get TL.
...
- If *p*(*i* - 1, *c*) = *x*~*i*~, then *p*(*i*, *c*) = *y*~*i*~

- Same, if *p*(*i* - 1, *c*) = *y*~*i*~, then *p*(*i*, *c*) = *x*~*i*~
...
Now we should take a look at how do stable elements affect their neighbors. Suppose *a*~*i* - 1~ = *a*~*i*~, meaning *i* - 1 and *i* are stable. Additionaly assume, that *a*~*i* + 1~ is not a stable element, hence *a*~*i* + 1~ ≠ *a*~*i*~ and *a*~*i* + 1~ ≠ *a*~*i* + 2~. Keeping in mind that only 0 and 1 values are possible, we conclude that *a*~*i*~ = *a*~*i* + 2~ and applying a median smoothing algorithm to this sequence will result in *a*~*i*~ = *a*~*i* + 1~. That means, if there is a stable element in position *i*, both *i* + 1 and *i* - 1 are guaranteed to be stable after one application of median smoothing. Now we can conclude, that all sequences will turn to stable at some point.

Note, that if there are two stable elements *i* and *j* with no other stable elements located between them, the sequence of elements between them is alternating, i.e. *a*~*k*~ = (*a*~*i*~ + *k* - *i*)*mod*2, where
...
The solution is: calculate *max*(*min*(|*i* - *s*~*j*~|)), where *s*~*j*~ are the initial stable elements. Time complexity is *O*(*n*).
...
If the velocity of the dirigible relative to the air is given by the vector (*a*~*x*~, *a*~*y*~), while the velocity of the wind is (*b*~*x*~, *b*~*y*~), the resulting velocity of the dirigible relative to the plane is (*a*~*x*~ + *b*~*x*~, *a*~*y*~ + *b*~*y*~).
...
The only thing we need to check now is that the distance between the point (*x*~*n*~, *y*~*n*~) and the target coordinates (*x*~2~, *y*~2~) can be covered moving with the speed *v*~*max*~ in
...
Let *i*~1~ < *i*~2~ <  ...  < *i*~*k*~ be the indices of the elements that will form the answer. Note, that the relative order of the chosen subset will remain the same, as there is no reason to swap two elements that will both be included in the answer. The minimum number of operations required to place this *k* elements at the beginning is equal to
...
The given problem actually consists of two separate problems: build the directed graph of substring relation and find the maximum independent set in it. Note, that if the string *s*~2~ is a substring of some string *s*~1~, while string *s*~3~ is a substring of the string *s*~2~, then *s*~3~ is a substring of *s*~1~. That means the graph of substring relation defines a partially ordered set.
...
Time complexity is *O*(*L* + *n*^3^) to build the graph plus *O*(*n*^3^) to find the maximum antichain. The overall complexity is *O*(*L* + *n*^3^).
