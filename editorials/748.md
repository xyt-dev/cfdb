...
Denote the two strings from the input by *s* and *t*. It's enough to find all pairs of distinct *s*~*i*~ and *t*~*i*~ and then do the following:

- Ensure that each symbol is in no more than one different pair,
- Ensure that if symbol *c* is in a pair with another symbol *d*, then each occurrence of *c* in *s* on the *i*-th place takes place iff *t*~*i*~ = *d*, and vice versa.
...
Solution #1: denote by *dp*~*i*~ the least possible number of points if the robot passed only *i* unit segments, and we assume that *dp*~0~ = 0. Thus, the answer to the problem is *dp*~*n*~. It's clear that *dp*~*k* + 1~ ≥ *dp*~*k*~ for every *k*, so for every *i* ≤ *n* one can obtain that *dp*~*i*~ = *dp*~*j*~ + 1, where *j* is the minimal of such indices that *s*[*j* + 1... *i*] can be a path from one point to another. That means two constraints:
