...
Let's match each element of *a* interval of values, which corresponding element of *c* could take, i.e for *i*-th element interval [*l* - *a*~*i*~;*r* - *a*~*i*~]. Let *pos*~*i*~ be the index of element equal to *i* in permutation *p*. Now you can know, that solving the initial task is reduced to picking a number of each interval, so that this numbers form an increasing sequence in order from *pos*~1~ to *pos*~*n*~. It is easy to know that we can pick the numbers greedily, picking a number that is greater than previous and belongs to current interval.
