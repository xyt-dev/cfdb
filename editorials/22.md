...
One may interest in much faster solution. Using simple DP solution one can get a solution with an O((n*m)<sup>2</sup>) complexity. It's clear, that rectangle with coordinates (x1, y1, x2, y2) is correct if and only if rectangles (x1, y1, x2-1, y2) and (x1, y1, x2, y2-1) are correct, and board[x2][y2] = '0'. So each of rectangles can be checked in O(1) and totally there will be O((n*m)<sup>2</sup>) operations.
