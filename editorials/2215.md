...
Idea by [Warriors_Cat](<https://codeforces.com/profile/Warriors_Cat>)
...
Idea by [E.Space](<https://codeforces.com/profile/E.Space>)
...
Idea by [Ecrade_](<https://codeforces.com/profile/Ecrade_>)
...
Greedily, it is never worse to choose the first interval to have length exactly $k$. Enumerate all intervals of length $k$, compute the result, and take the minimum. The final answer is:
 $ \min\limits_{r-l+1=k}\left(\sum\limits_{i\not\in[l,r]}\min(b_i,c_i)+\min\left(\sum\limits_{i\in[l,r]} b_i,\sum\limits_{i\in[l,r]} c_i\right)\right) $
...
Idea by [SpiritualKhorosho](<https://codeforces.com/profile/SpiritualKhorosho>)
...
int main(){
	...
	while (T--) {
		...
		for (i = 3; 1ll * i * i < n; ++i) {
			if (n % i == 0) {
				ans += check(n, i &mdash; 1, 2);
				ans += check(n, 1ll * n / i &mdash; 1, 2);
			}
		}
		if (1ll * i * i == n) ans += check(n, i &mdash; 1, 2);
		for (i = 3; i <= 63; ++i) {
			for (j = 2; j <= 1000000; ++j) {
				sum = pow = 1;
				for (k = 1; k < i; ++k) {
...
Idea by [dapingguo8](<https://codeforces.com/profile/dapingguo8>)
...
Idea by [xiaoziyao](<https://codeforces.com/profile/xiaoziyao>)
...
Idea by [Kratrissa](<https://codeforces.com/profile/Kratrissa>)
...
Thus, as long as it is not one of the three cases $n \equiv k, n \equiv k+1, n-x \equiv k-1$, we have $f_k(n) = f_k(n-x) + 1$. For the third case, since $f_k(n-x-1)$ must be greater than or equal to $f_k(n-x+1) = 1$, it can be expressed as $1 + \min\left( f_k(n - \left\lfloor \frac{n+k}{2k} \right\rfloor), f_k(n - \left\lfloor \frac{n+k}{2k} \right\rfloor - 1) \right)$. Thus, we have the following recurrence relation:
 $f_k(n) = \begin{cases} 1, & n \equiv k \pmod{2k} \\ 1 + \min\left\{f_k\left(n - \left\lfloor \frac{n+k}{2k} \right\rfloor\right), f_k\left(n - \left\lfloor \frac{n+k}{2k} \right\rfloor + 1\right)\right\}, & n \equiv k+1 \pmod{2k} \\ 1 + \min\left\{ f_k\left(n - \left\lfloor \frac{n+k}{2k} \right\rfloor\right), f_k\left(n - \left\lfloor \frac{n+k}{2k} \right\rfloor - 1\right) \right\}, & n \equiv k-1 + \left\lfloor \frac{n+k}{2k} \right\rfloor \not\equiv k+1 \pmod{2k} \\ 1 + f_k(n - \left\lfloor \frac{n+k}{2k} \right\rfloor), & \text{otherwise} \end{cases}$
...
Idea By [crazy_sea](<https://codeforces.com/profile/crazy_sea>)
