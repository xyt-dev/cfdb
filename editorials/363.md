...
. Then the sum of heights of the planks *i*, *i* + 1, ..., *i* + *k* - 1 is *s*~*i* + *k* - 1~ - *s*~*i* - 1~. The other approach is to calculate the sum of the first *k* planks: *h*~1~ + *h*~2~ + ... + *h*~*k*~. By subtracting *h*~1~ and adding *h*~*k* + 1~ we get sum of *k* planks starting from the second plank. Then, by subtracting *h*~2~ and adding *h*~*k* + 2~ we get sum of *k* planks starting from the third plank, and so on.
## 363C - Fixing Typos
