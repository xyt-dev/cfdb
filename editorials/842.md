...
It's easy to see that if the number written on some vertex *i* is not equal to 0, then its beauty will be some divisor of *a*~*i*~. Also if the number written on the root is 0 then the beauty of each vertex can be easily calculated. Otherwise beauty of each vertex will be a divisor of the number in the root.

Let's calculate the beauty of each vertex if the number in the root is 0. This can be done by traversing the tree, and the beauty of *i* is *gcd*(*a*~*i*~, *ans*[*par*~*i*~]).
...
If the last query was *x*~*i*~ and then we receive a query *x*~*i* + 1~, then we can leave the original array unchanged and use the number

![](eimages/842_4.png)

 as the second query. So we will maintain current xor of queries instead of changing the array.

It's easy to see that if the array contains all numbers from zero to 2^*k*^ - 1 and the number in the query is less than 2^*k*^, then the array will still contain all those numbers.
