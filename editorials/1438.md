<!-- url: https://codeforces.com/blog/entry/84589 -->
...
- [Problem A and B](<https://www.youtube.com/watch?v=Z0JKnlmhVOc>)
...
We will first solve the problem for odd $n$, and then extend the solution to even $n$.
...
- While at least $3$ unpaired elements exist, apply the operation on any $3$.
- Pair any two of them and repeat.
 Finally, we will have just one unpaired element left, using which we can make all elements equal as described at the start.
The number of operations used is exactly $n-1$.
...
Since the xor of an even number of same elements is $0$, the answer is impossible for arrays with $X \neq 0$.
...
This problem was set by [Anti-Light](<https://codeforces.com/profile/Anti-Light>) and prepared by [knightron00](<https://codeforces.com/profile/knightron00>)

## 1438E - Yurii Can Do Everything
...
- Fix the left endpoint $l$. Let $k$ be the most significant set bit in $a_l$.
- Check every $r$ in increasing order by bruteforce while $\text{sum}(l+1,r-1)$ is smaller than $2^{k+1}$.
