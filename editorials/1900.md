<!-- url: https://codeforces.com/blog/entry/122677 -->
...
First remove digits $2$ and $3$ and write digit $1$ while $b \gt 0$ and $c \gt 0$. If $b=c$, then we are done. Otherwise, without loss of generality assume $b \gt c$. That means that after the operations $c=0$ and $b$ is even (Because $b$ and $c$ are the same parity). Now we perform the following $2$ operations $\frac{b}{2}$ times to get only digits $1$ left. Remove digits $1$ and $2$ and add digit $3$. After that remove digits $2$ and $3$ and add digit $1$. An effective change of these $2$ operations is the reduction of $b$ by $2$.
...
Author: [Riblji_Keksic](<https://codeforces.com/profile/Riblji_Keksic>)
