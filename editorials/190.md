...
In this problem we have an array of strings (*s*~1~, *s*~2~, ...*s*~*n*~), where *s*~*i*~  = *pair* or *int*.
Let's consider *bal*~*i*~ = the difference between number of "pair" and "int" int the subarray (*s*~1~, *s*~2~, ...*s*~*i*~).
Than we can prove that the type can be reestablished from the array *s*  <  =  >  *bal*~*i*~ >  = 0 for 1 <  = *i* < *n* and *bal*~*n*~ =  - 1. This can be proved using mathematical induction, the parameter is the number of "*int*"-s in the array.
And how to find the solution, if we know that is exists? Consider the function *gettype*(*inti*), which builds the type beginning in the position *i* and returns the index, next to the position where it finished building of type. How it works? If *s*~*i*~ =  "*int*", then the function prints "*int*" and returns *i* + 1. Else it prints "*pair* < ", *gettype*(*i* + 1) is launched, let it returns *j*. Then we print ", " and launch *gettype*(*j*) (it returns *k*), then we print " > " and return *k*.
