<!-- url: https://codeforces.com/blog/entry/100127 -->
...
You can always pick the $2$ maximum weights: if they are $a_i$ and $a_j$ ($i \lt j$), you can flip the subsegment $[i, j-1]$ to make them adjacent.
...
Official solution: [150288088](<https://codeforces.com/contest/1654/submission/150288088>)

## 1654B - Prefix Removals

Author: [emorgan](<https://codeforces.com/profile/emorgan>)
 Preparation: [TheScrasse](<https://codeforces.com/profile/TheScrasse>)
...
Can you ever remove $a_i$, if $a_i\not=a_{i+1}, a_{i+2}, \dots, a_n$? Notice that you would need to remove $a(l, r)$ ($l \leq i \leq r$): this means that there must exist $a(l', r') = a(l, r)$ for some $l' \gt l$. So, $a_{i+l'-l} = a_i$, and this is a contradiction.
...
Official solution: [150288210](<https://codeforces.com/contest/1654/submission/150288210>)
...
Official solution: [150288232](<https://codeforces.com/contest/1654/submission/150288232>)
...
Assume that the amount of ingredient $1$ is $1$. For each $i$, the amount of ingredient $i$ would be $c_i/d_i$ for some integer $c_i, d_i \gt 0$. How to make an integer amount of each ingredient?
...
Official solution: [150288255](<https://codeforces.com/contest/1654/submission/150288255>)
...
Let $m$ be the upper bound on $a_i$. Let $d$ be the difference between adjacent elements in the final array. Create a piecewise algorithm for the cases where $|d| \lt \sqrt m$ and $|d| \geq \sqrt m$
...
For $d \lt \sqrt m$, we can use the above algorithm to handle all of these $d$ in $O(n \sqrt m)$ time. We can keep a hashmap from bucket index $\to$ number of elements in the bucket, or we can just keep an array since the bucket indices have a range of at most $O(n \sqrt m)$ which is not enough to exceed the memory limit.

For $d \geq \sqrt m$, we observe that if we have two indices $i, j$ such that $j \gt i+\sqrt m$, then at least one of them definitely has to have an operation performed on it, because the difference between them would have to be $a_j-a_i \geq \sqrt m \cdot d \gt m$ which is not possible. In other words, if we consider the set of elements which are not edited, that set will have gaps between pairs of elements of size at most $\sqrt m$.

So, we can build a graph between indices with an edge $i \to j$ with label $x$ if $i \lt j \leq i+\sqrt m$ and $\frac{a_j-a_i}{j-i} = x$. This graph has at most $n\sqrt m$ edges. Then we just have to find the longest path in the graph where all edges have the same label. You can do this with dynamic programming -- let $dp_{i,d}$ be the length of the longest path ending at index $i$, where all edges have label $d$. For each $i$, we only need to check edges to $j$ where $i - \sqrt m \lt j \lt i$. This means the time complexity is $O(n\sqrt m)$. To store the values $dp_{i,d}$ sparsely, we can use either a hash map or a rotating buffer (where we only store $dp_{i,d}$ for $i$ in a sliding window of width $\sqrt m$).

Complexity: $O(n \sqrt m)$

Official solution: [150288285](<https://codeforces.com/contest/1654/submission/150288285>)

## 1654F - Minimal String Xoration

Author: [dario2994](<https://codeforces.com/profile/dario2994>), [emorgan](<https://codeforces.com/profile/emorgan>)
 Preparation: [dario2994](<https://codeforces.com/profile/dario2994>), [TheScrasse](<https://codeforces.com/profile/TheScrasse>)

**Hint 1**

Let $f(s, x)$ be the string $t$ such that $t_i = s_{i\oplus x}$. The intended solution not only finds the $x$ such that $f(s, x)$ is lexicographically minimized, but produces an array of all $0 \leq x \lt 2^n$ sorted according the comparator $f(s, i) \lt f(s, j)$.
...
In order to construct $a_k$ from $a_{k-1}$, we will first construct an auxiliary array of integers $v$ using $a_{k-1}$, where $v_i \lt v_j$ iff $f(s, i)_{0..2^{k-1}} \lt f(s, j)_{0..2^{k-1}}$. Then, we will sort the array $a_k$ according to the comparison of tuples $(v_i, v_{i\oplus {2^{k-1}}}) \lt (v_j, v_{j\oplus {2^{k-1}}})$.
...
Official solution: [150288326](<https://codeforces.com/contest/1654/submission/150288326>)

## 1654G - Snowy Mountain

Author: [emorgan](<https://codeforces.com/profile/emorgan>)
 Preparation: [dario2994](<https://codeforces.com/profile/dario2994>), [emorgan](<https://codeforces.com/profile/emorgan>), [TheScrasse](<https://codeforces.com/profile/TheScrasse>)
...
Official solution: [150288345](<https://codeforces.com/contest/1654/submission/150288345>)

## 1654H - Three Minimums

Author: [dario2994](<https://codeforces.com/profile/dario2994>), [TheScrasse](<https://codeforces.com/profile/TheScrasse>)
 Preparation: [dario2994](<https://codeforces.com/profile/dario2994>), [TheScrasse](<https://codeforces.com/profile/TheScrasse>)
...
- If $p_1=1$ and $p_n=2$, then $p$ is good if and only if it is bitonic, i.e., $p_1 \lt p_2 \lt p_3 \lt \cdots \lt p_i \gt p_{i+1} \gt \cdots p_{n-1} \gt p_n$, where $p_i=n$.

Given $1\le l \lt r\le n$, we say that a permutation $q_1,q_2,\dots, q_{r-l+1}$ of ${1,2,\dots, r-l+1}$ is *$[l,r]$-consistent* if for any $l\le i \le \min(r, m-1)$:


- $q_{i-l+1} \lt q_{i-l+2}$ if $s_i = \texttt{ \lt }$;

- $q_{i-l+1} \gt q_{i-l+2}$ if $s_i = \texttt{ \gt }$.
...
For $1\le l \lt r\le n$, let $a_{\ast\ast}(l, r)$, $a_{1\ast}(l, r)$, $a_{\ast 1}(l, r)$, $a_{12}(l, r)$, $a_{21}(l, r)$ be the the number of good permutations which are $[l,r]$-consistent and, respectively,
...
For $1\le i \lt n$ and $c\in{\texttt{ \lt }, \texttt{ \gt }}$, let $q(i, c) := [i \gt m\text{ or } s_i= c]$. Informally speaking, $q(i, \texttt{ \lt })$ is $1$ if and only if it can be $p_i \lt p_{i+1}$ and $q(i, \texttt{ \gt })$ is $1$ if and only if it can be $p_i \gt p_{i+1}$.
...
- $a_{1\ast}(l, l) = 1$. For $l \lt r$, $a_{1\ast}(l, r) = \sum_{i=l+1}^r a_{12}(l, i)a_{1\ast}(i, r)\binom{r-l-1}{i-l-1}$.

- Analogous formula for $a_{\ast 1}$.

- $a_{12}(l, l) = 0$ and $a_{12}(l, l+1)= q(l, \texttt{ \lt })$ and $a_{12}(l, l+2)= q(l, \texttt{ \lt })\cdot q(l+1, \texttt{ \gt })$. For $l \lt l+1 \lt r$, $a_{12}(l, r) = q(l, \texttt{ \lt })a_{21}(l+1, r) + q(r-1, \texttt{ \gt })a_{12}(l, r-1)$.
...
We have the recurrence relation (for $k\ge 2$)
 $\tag{1} a_{\ast1}(1, k) = \sum_{i=1}^k a_{\ast1}(1, i) a_{21}(i, k) \binom{k-2}{i-1} $
Setting $x_{k-1} := a_{\ast1}(1, k) / (k-1)!$, (1) is equivalent to (for $k\ge 1$, and also for $k=0$!)
 $\tag{2} k\cdot x_k = \sum_{i=0}^{k-1} x_i \frac{a_{21}(i+1, k+1)}{(k-1-i)!}. $
...
Notice that, if we set $b$ to any of the functions $a_{\ast\ast}$, $a_{\ast1}$, $a_{1\ast}$, $a_{12}$, $a_{21}$, it holds $b(l, r) = b(l+1, r+1)$ whenever $l \gt m$. Hence, let us define $b_{\ast\ast}(k) = a_{\ast\ast}(n+1, n+k)$ and analogously $b_{1\ast}(k)$, $b_{\ast 1}(k)$, $b_{12}(k)$, $b_{21}(k)$.

With these new definitions, (2) becomes (for $k\ge 0$)
 $\tag{3} k\cdot x_k = \sum_{i=0}^{k-1} x_i \frac{b_{21}((k-1-i) + 2)}{(k-1-i)!} + \sum_{i=0}^{\min(k-1, m-1)} x_i \frac{a_{21}(i+1, k+1) - b_{21}(k+1-i)}{(k-1-i)!} . $
Let $u_i:= \frac{b_{21}(i+2)}{i!}$ and $v_{k-1}:= \sum_{i=0}^{\min(k-1, m-1)} x_i \frac{b_{21}(k+1-i) - a_{21}(i+1, k+1)}{(k-1-i)!}$. So, (3) simplifies to
 $ \tag{4} k\cdot x_k = v_{k-1} + \sum_{i=0}^{k-1} x_i u_{k-1-i} .$
We precompute in $O(nm)$ the values of $a_{12}(l, r)$ and $a_{21}(l, r)$ for $1\le l\le m$, $l \lt r\le n$. We can also precompute in $O(n)$ the values of $b_{12}(k), b_{21}(k)$ for $1\le k\le n$. In $O(m^2)$ we compute also $x_i$ for all $0\le i\le m-1$. Thus, in $O(nm)$ we can compute, for all $0\le k \lt n$, the values $u_k$.
...
It is now time to start working with generating functions. Let $X(t):= \sum_{k\ge 0} x_k t^k$, $U(t):=\sum_{k\ge0} u_kt^k$, $V(t):=\sum_{k\ge0} v_kt^k$. We know $U(t)$ and $V(t)$ (at least the first $n$ coefficients) and we want to compute $X(t)$. Since $x_0=1$, we know $X(0)=1$. Moreover (4) is equivalent to the ordinary differential equation $X' = V + U\cdot X$. This ODE is standard and its (unique) solution is given by
 $ X = \exp(\smallint U)\Big(1 + \int V\exp(-\smallint U)\Big).$
Since the product of generating functions and the exponential of a generating function can be computed in $O(n\log(n))$, we are able to obtain the values of $x_k$ for all $0\le k \lt n$ and thus the values of $a_{\ast 1}(1,k)$.

Now, let us see how to compute $a_{1\ast}(k, n)$. Since $a_{1\ast}(k,n) = b_{1\ast}(n-k+1)$ for all $m \lt k\le n$, let us first compute $b_{1\ast}(k)$ for all $1\le k\le n$. By repeating verbatim the reasoning above, we get that the generating function $Y(t):=\sum_{k\ge 0} y_k t^k$, where $y_{k-1}:=b_{1\ast}(k) / (k-1)!$, is given by ($V=0$ in this case) $Y=\exp(\int U)$. So, it remains only to compute $a_{1\ast}(k, n)$ for $1\le k\le m$. This can be done naïvely in $O(nm)$.

The overall complexity is $O(nm + n\log(n))$.

Official solution: [150306974](<https://codeforces.com/contest/1654/submission/150306974>)
