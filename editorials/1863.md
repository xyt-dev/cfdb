<!-- url: https://codeforces.com/blog/entry/119902 -->
...
Let $p$ be the total number of + signs in the notification string. If $a + p \lt n$, then the answer is clearly "NO".
...
For every $k$ such that $a_i = k$, $a_j = k + 1$ and $i \gt j$ we have to choose $x = k + 1$ at least once for these elements to be in the correct order.
...
- When building all $c_i$ the first time, also find for each vertex $v$ the first $x$ when its $c_v$ must change. If we denote it with $w_v$ then it can be expressed as $\max\{w_u\,\colon\,u\to v, c_u + k \gt c_v\}$. Then sort all events of type "some $c_i$, where quest $i$ may or may not have incoming dependencies, increase by $k$".
...
To check that we can achieve $l$, $r$ we need to check that $s \land mask\_start_l \gt 0$ or $s \land mask\_end_r \gt 0$ or any segment can start/end in these positions, since there was a segment with zero XOR with such start/end.
...
We'll maintain a set $S$ of pairs $(v, D)$, where $D$ is a lower bound on $D_v$. Initially, put pairs $(v,(a_v - 1)\cdot 2^{h_v})$ into $S$ for all leaves with $a_v \gt 0$, and if there are $(v, D)$ and $(u, D)$ in $S$ ($v \neq u$), put $(w, D + 2^{h_w})$ into $S$, where $w = \text{lca}(v, u)$. The answer is $1 +$ the maximum $D$ among all pairs in $S$ (if $S$ is empty, the answer is obviously $0$).
...
- If $cd$ is contained within a longer path, it must start at $c$ (since all other paths are also contained in $ab$). In this case, we can extend $ab$.
 We will refer to classes of adjacent paths as simply classes. Each class will either be taken in its entirety or not taken at all, and according to the proposition above, an optimal solution can always be found.
...
Proposition. Let class $a$ be an ancestor of class $b$. Then, a pair of nested paths between them can exist if and only if for all classes $c$ on the path between $a$ and $b$ (including $b$ but excluding $a$), $L(c) \gt L(a)$.
