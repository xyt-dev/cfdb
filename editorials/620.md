...
Easy to see that the answer is *max*(|*x*~1~ - *x*~2~|, |*y*~1~ - *y*~2~|).
...
We can process the cases of zero or one swap in *O*(*nm*) time. Consider the case with two swaps. Note we can assume that two swaps will lead to move two elements from *a* to *b* and vice versa (in other case it is similar to the case with one swap). Let's iterate over all the pairs of the values in *a* and store them in some data structure (in *C++* we can user *map*). Now let's iterate over all the pairs *b*~*i*~, *b*~*j*~ and find in out data structure the value *v* closest to the value *x* = *s*~*a*~ - *s*~*b*~ + 2·(*b*~*i*~ + *b*~*j*~) and update the answer by the value |*x* - *v*|. Required sum we can find using binary search by data structure (*map* in C++ has *lower_bound* function).
