...
In this problem you had to simulate route of character in graph.
 Note that if you are in vertice *i*, then edges in all vertices with numbers less than *i* are turned to *p*~*i*~. It gives us opportunity to see a recurrence formula: let *dp*~*i*~ be number of steps, needed to get from vertice 1 to vertice *i*, if all edges are rotated back, into *p*~*i*~. Then *dp*~*i* + 1~ = 2*dp*~*i*~ + 2 - *dp*~*p*~*i*~~. Answer will be *dp*~*n* + 1~.

**BONUS**: Can you solve this task without statement *p*~*i*~ ≤ *i*? I don't know the solution, it seems difficult.
...
If *d* ≠ 0, then notice that if on some subsegment there are two numbers *a*~*i*~, *a*~*j*~ so that *a*~*i*~%*d* ≠ *a*~*j*~%*d*, then this segment can't be good.
