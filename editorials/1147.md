<!-- url: https://codeforces.com/blog/entry/66878 -->
Here's the tutorial. Code can be found in this link (more will be added there soon): [https://www.dropbox.com/sh/xwxn3zrl1icp3i2/AACtYSdYH0KlTEdVgCFFgPrYa?dl=0](<https://www.dropbox.com/sh/xwxn3zrl1icp3i2/AACtYSdYH0KlTEdVgCFFgPrYa?dl=0>)

**Zoning Restrictions Again**

## 1162A - Zoning Restrictions Again
...
First, let's swap $a_{i,j}$ with $b_{i,j}$ if $a_{i,j} \gt b_{i,j}$. At the end, for every $i$ and $j$, we have $a_{i,j} \leq b_{i,j}$. We now claim that there is a solution if and only if this configuration is valid. We can guess this intuitively and by trying a few examples, or we can do the proof below.
...
To answer this question efficiently, let's precompute the first and last occurrence of each number. This can be done by sweeping through the array once. Thus, checking if the condition is satisfied is just checking that $first[p] \gt last[q]$.
...
Since the leading character of $s$ is a "1", then that means $a \lt 2^{m-1}$ and $2^{m-1} \leq b \lt 2^m$. Let's fix the length of $a$ as $k$. I'll describe a more general solution, so there might be simpler solutions that work for this specific problem.
...
- For the $i$-th bit, if $s_i$ is "1", draw a $1$ edge between $a_i$ and $b_i$ (if $i \gt k$, we instead draw an edge from $b_i$ to $1$). If $s_i$ is "0", then draw a $0$ edge between $a_i$ and $b_i$. If $s_i$ is "?", then don't draw any edges, since there are no explicit constraints.
...
Let's see what this means in terms of edge weights. If Alice can move from $x$ to $y$, then that means $edge(x,y) \gt edge(w,x)$, since the last edge traversed was $(w,x)$ and Alice is increasing. We want it to be the case that $edge(x,y) \gt edge(w,x)$ implies that $edge(x,y) \gt edge(y,z)$. Thus, the matching is bad if and only if there are two edges in the matching $(w,x)$ and $(y,z)$ such that $edge(x,y) \gt edge(w,x)$ and $edge(x,y) \lt edge(y,z)$.
