<!-- url: https://codeforces.com/blog/entry/70008 -->
## 1230A - Давид и мешки с конфетами
...
## 1229A - Marcin and Training Camp
...
## 1229B - Kamil and Making a Stream
...
## 1229C - Конрад и оценка компании
...
## 1229E1 - Марек и паросочетания (упрощенная версия)
...
## 1229E2 - Марек и паросочетания (усложненная версия)
...
Let's introduce the variables $x_1, x_2, \dots, x_n$ where $x_i$ is the number of coins passed from the $i$-th to the $(i+1)$-th scale (or, $x_i \lt 0$, it means that $-x_i$ coins are passed from the $(i+1)$-th to the $i$-th scale). We can now create the following conditions regarding the final number of stones on each scale:
...
- If $x_1, x_2, \dots, x_n \gt 0$, then we take any coin and make a full lap with it along the circle in the order of increasing $i$'s. We can now decrease each $x_i$ by one.
- If $x_1, x_2, \dots, x_n \lt 0$, we can do the similar thing, but we're decreasing $i$'s.
- In the remaining cases, we can pick a scale $i$ that won't receive coins anymore (that is, $x_i \geq 0$ and $x_{i-1} \leq 0$) and it still has some coins to distribute ($x_i \gt 0$ or $x_{i-1} \lt 0$). If $x_i \gt 0$, take a single coin, put it on the $(i+1)$-th scale, and decrease $x_i$ by one. If $x_{i-1} \lt 0$, take a coin, put it on the $(i-1)$-th scale, and increase $x_i$ by one.
