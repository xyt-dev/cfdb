...
You need to implement what written in statement. You could act like that: let's calculate *q*~*i*~ — minimum item price from seller *i*. Then if *q*~*i*~ < *v*, we can make a deal with seller *i*, otherwise we can't.
...
**Bonus**. Suppose fruit can be collected at days *a*~*i*~, *a*~*i*~ + 1, ..., *a*~*i*~ + *T*~*i*~, where *T*~*i*~ — some number for each tree. How to solve this task optimally?
...
. As soon as every cycle is broken with linear asymptotics, this case solution works with *O*(*n*^2^).
