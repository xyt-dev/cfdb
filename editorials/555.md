...
Time: *O*(*n*) or *O*(*n*^2^); solutions: [O(n)](<http://paste.ubuntu.com/11788578/>) and [O(n^2)](<http://paste.ubuntu.com/11788583/>)
...
We can put a bridge between bridges *i* and *i* + 1 if its length lies in the segment [*l*~*i* + 1~ - *r*~*i*~;*r*~*i* + 1~ - *l*~*i*~]. Now we have a well-known problem: there are *n* - 1 segments and *m* points on a plane, for every segment we need to assign a point which lies in it to this segment and every point can be assigned only once.
...
Time: *O*(*n* + *ql*~*q*~) where *l*~*q*~ is the time of calculating LCA per query; [solution](<http://paste.ubuntu.com/11902227/>) that uses slightly other method for the last part.
