<!-- url: https://codeforces.com/blog/entry/70898 -->
...
## 1225B1 - TV Subscriptions (Easy Version)
...
## 1225B2 - TV Subscriptions (Hard Version)
...
We can now check all values of $k$ starting from the smallest. If $n \geq 31p + 31$, then the answer will not exceed $31$ since $n - 31p$ is less than $2^{31}$, hence is always representable with $31$ powers. Otherwise, we have $n - 31(p + 1) \lt 0$. Since $n \gt 0$, it means that $p + 1 \lt 0$, and $n - kp \lt k$ for all $k \gt 31$, thus the answer does not exist.
