...
. So *L*~*i*~ ≤ *r*~*i*~ ≤ *R*~*i*~. This gives us a L-R max-flow approach: aside these vertices, add a source *S* and a sink *T*. For every vertex *v* in part *X*, add an edge with minimum and maximum capacity *L*~*v*~ and *R*~*v*~ from *S* to *v*. For every vertex *u* in part *Y*, add an edge with minimum and maximum capacity *L*~*u*~ and *R*~*u*~ from *u* to *T*. And finally for every edge *v* - *u* from *X* to *Y* add an edge from *v* to *u* with capacity 1 (minimum capacity is 0).
