...
For pedestrian crossing *i* (1 ≤ *i* ≤ 4), lanes *l*<sub>*i*</sub>, *s*<sub>*i*</sub>, *r*<sub>*i*</sub>, *s*<sub>*i* + 2</sub>, *l*<sub>*i* + 1</sub>, *r*<sub>*i* - 1</sub> are the only lanes that can cross it. So, we have to check that either *p*<sub>*i*</sub> = 0 or all mentioned lanes are 0.
...
For query *x* *y*, if toy *y* is free, then child *x* can take it and no child will cry. Otherwise, toy *y* is held by another child. Lets denote *z* to be the last child who requested toy *y*. So *x* now depends on *z*. If *z* is in the subtree of *x*, then all children in the subtree of *x* will cry. Otherwise, no child will cry. We can check that a node is in the subtree of another node using euler walk (*t*<sub>*in*</sub> and *t*<sub>*out*</sub>) with preprocessing in *O*(*n*) and query time *O*(1)
