<!-- url: https://codeforces.com/blog/entry/155840 -->
...
## 2255A-Hot Potatoes at the Fairy Warehouse
...
## 2255D-How Long Until Nothing Remains?
...
[2255E1-What Will Remain at the End? (Easy Version)](<https://codeforces.com/contest/2255/problem/E1>) Idea: [paulzrm](<https://codeforces.com/profile/paulzrm>)
...
[2255E2-What Will Remain at the End? (Hard Version)](<https://codeforces.com/contest/2255/problem/E2>) Idea: [paulzrm](<https://codeforces.com/profile/paulzrm>)
...
## 2255F-Who Will Witness the End?
...
Comparing each monomial type gives
 $ c_k=\sum_{r=k}^m\binom{n-2k}{r-k}h_r \qquad(0\le k\le m). $
...
For $k\ge1$, divide two adjacent coefficients:
 $ \begin{aligned} \frac{c_{k+1}}{c_k} &=\frac{(k+1)!k!}{(2k+1)!} \frac{(2k-1)!}{k!(k-1)!}\\ &=\frac{k(k+1)}{(2k)(2k+1)}\\ &=\frac{k+1}{2(2k+1)}. \end{aligned} $
The $k=0$ case follows directly from $c_0,c_1$. Hence
 $ (4k+2)c_{k+1}-(k+1)c_k=0. $
Define
 $ L_k=(k+1)c_k-(4k+2)c_{k+1}=0 \qquad(0\le k \lt m). $
...
Now use
 $ jA_j=(N-j-1)A_{j-1},\qquad (N-j)A_{j-2}=(j-1)A_{j-1}, $
together with $r=k+j$ and $n=N+2k$. The entire coefficient calculation can be displayed as
 $ \begin{aligned} [h_r]L_k &=(k+1)\binom Nj-(4k+2)\binom{N-2}{j-1}\\ &=(k+1)(A_j+2A_{j-1}+A_{j-2})-(4k+2)A_{j-1}\\ &=(k+1)A_j-2kA_{j-1}+(k+1)A_{j-2}\\ &=(r+1)A_j-(n-2)A_{j-1}+(n-r+1)A_{j-2}. \end{aligned} $
Substituting into $L_k$ and shifting the latter two indices gives
 $ 0=L_k=\sum_{r=k}^m\binom{n-2k-2}{r-k} \left((r+1)h_r-(n-2)h_{r+1}+(n-r-1)h_{r+2}\right), $
...
The highest two terms come directly from the original system:
 $ h_m=c_m,\qquad h_{m-1}=c_{m-1}-(n-2m+2)h_m. $
