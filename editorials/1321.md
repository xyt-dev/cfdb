<!-- url: https://codeforces.com/blog/entry/74431 -->
**Div2A**
...
Consider the last case — there is at least one letter $s_k$ between $s_i$ and $s_j$. Because we cannot remove $s_k$ then there are only two cases: $s_k \gt s_j + 1$ or $s_k + 1 \lt s_i$. Then we cannot use $s_i$ to remove $s_j$ at all.
...
- the rebuild will definitely occur if $d_u \gt d_v − 1$;
...
## 1320C - World of Darkraft: Battle for Azathoth
...
- Iterate on weapons and monsters in sorted order (sort by $a_i$/$x_k$ in non-descending order). For each new weapon $i$ we should add all monsters $x_k \lt a_i$ which were not added yet (we can maintain a pointer on the last added monster to find them quickly).
