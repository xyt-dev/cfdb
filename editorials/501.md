...
Note that every non-empty forest has a leaf(vertex of degree 1). Let's remove edges one by one and maintain actual values (*degree*<sub>*v*</sub>, *s*<sub>*v*</sub>) as long as graph is not empty. To do so, we can maintain the queue(or stack) of the leaves. On every iteration we dequeue vertex *v* and remove edge (*v*, *s*<sub>*v*</sub>) and update values for vertex *s*<sub>*v*</sub>: *degree*<sub>*s*<sub>*v*</sub></sub> -= 1 and *s*<sub>*s*<sub>*v*</sub></sub> ^= *v*. If degree of vertex *s*<sub>*v*</sub> becomes equal to 1, we enqueue it.
...
Firstly, we convert each number into a binary system: it can be done in *O*(*MAXBITS*<sup>2</sup>), where *MAXBITS* ≤ 2000 with rather small constant(we store number in system with big radix).
