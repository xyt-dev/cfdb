...
For each string *s*<sub>*i*</sub> iterate *j* from 1 to *i* - 1.

Check if *s*<sub>*i*</sub> = *s*<sub>*j*</sub> for any value of *j*.
...
Create a dynamic programming table of size *n*·3. In this, *dp*[*i*][0] stores maximum of value *p*·*a*<sub>*x*</sub> for *x* between 1 and *i*. Similarly *dp*[*i*][1] stores the maximum value of *p*·*a*<sub>*x*</sub> + *q*·*a*<sub>*y*</sub> such that *x* ≤ *y* ≤ *i* and *dp*[*i*][2] stores maximum value of *p*·*a*<sub>*x*</sub> + *q*·*a*<sub>*y*</sub> + *r*·*a*<sub>*z*</sub> for *x* ≤ *y* ≤ *z* ≤ *i*.

To calculate the dp:

*dp*[*i*][0] = *max*(*dp*[*i* - 1][0], *p*·*a*<sub>*i*</sub>)

*dp*[*i*][1] = *max*(*dp*[*i* - 1][1], *dp*[*i*][0] + *q*·*a*<sub>*i*</sub>)

*dp*[*i*][2] = *max*(*dp*[*i* - 1][2], *dp*[*i*][1] + *r*·*a*<sub>*i*</sub>)
...
Then iterate *j* through the array. Calculate *sum* = *q*·*a*<sub>*j*</sub>. Now, if *p* < 0, add *minLeft*<sub>*i*</sub>·*p* to *sum*. Otherwise add *maxLeft*<sub>*i*</sub>·*p* to *sum*. Similarly, if *r* > 0, add *maxRight*<sub>*i*</sub>·*r*, otherwise add *minRight*<sub>*i*</sub>·*r* to *sum*.
...
Now, to find the number of integers less than equal to *l* - 1 with *mask* = 0, we first add all the integers with *mask* = 0 who have length less than length of *l* - 1 in base *b* representation. If length of *l* - 1 in base *b* is *l*<sub>*b*</sub>, this value can be calculated as

![](eimages/855_3.png)

. The second term is subtracted to take into account the trailing zeros.

Now, we need to calculate the number of integers with length = *l*<sub>*b*</sub> and value ≤ *l* - 1 and *mask* = 0. Let the number *l* - 1 in base *b* representation be *l*<sub>0</sub>, *l*<sub>1</sub>... *l*<sub>*l*<sub>*b*</sub></sub>. Then, if we fix the first digit of our answer, *x* from 0 to *l*<sub>0</sub> - 1, we can simply calculate the mask for remaining digits we need as 2<sup>*x*</sup> and thus adding *dp*[*b*][2<sup>*x*</sup>][*len* - 1] to answer.

Now, if we fix the first digit as *l*<sub>0</sub> only, we can simply perform the same operation for the second digit, selecting value of second digit, *y* from 0 to *l*<sub>1</sub> - 1, and thus adding
...
This problem can be solved by using the concept of square root decomposition. Let us divide the total range of x-axis, that is, from 1 to 10<sup>5</sup> in
...
, and number of blocks, *n*<sub>*b*</sub> = 10<sup>5</sup> / *blockSize*. Now, we maintain the following structures:
...
- Arrays *minup* and *mindown* for each value from 1 to 10<sup>5</sup>. *minup* stores the minimum value for *y* such that *y* > 0 and there is a snake at that point and that snake does not cover the complete block. Similar value for *y* < 0 is stored by *mindown*
...
If we take *n* = 10<sup>5</sup>, the expected complexity of the solution is
