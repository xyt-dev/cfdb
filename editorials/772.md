...
For each device and a fixed time *T*, we can compute the rate that we need to charge it so that the device always has positive power. Well, it uses *a*<sub>*i*</sub> power per second, so it uses a total of *T* * *a*<sub>*i*</sub> power. It currently has *b*<sub>*i*</sub> power, so we need *X*<sub>*i*</sub> = *max*(0, *T* * *a*<sub>*i*</sub> - *b*<sub>*i*</sub>) units of power. This means we need *X*<sub>*i*</sub> / *T* units of power per second to this device.
...
Since we are binary searching on doubles, it is useful to just do it for a fixed number of steps. We can approximate the max answer is somewhere around 10<sup>14</sup> and we need a precision of 10<sup> - 4</sup>, so we need approximately
...
Let f(freq) taken in a frequency array of length 10^k, and returns a list of length 10^k, where the i-th element represents the result. When *k* = 0, this is easy to compute, it is just 2<sup>*freq*[0]</sup>.
...
Now, suppose want to add another leaf *w*<sub>*i*</sub>.
