<!-- url: https://codeforces.com/blog/entry/65905 -->
(Idea of the problem A — [----------](<https://codeforces.com/profile/---------->))
...
(Idea of the problem B — [IbragiMMamilov](<https://codeforces.com/profile/IbragiMMamilov>))
...
(Idea of the problem C — [usertab34](<https://codeforces.com/profile/usertab34>))
...
[Code](<https://pastebin.com/mzMFdXrk>)

(Idea of the problem D — [Denisson](<https://codeforces.com/profile/Denisson>))
...
[Code](<https://pastebin.com/JGRSfHid>)

(Idea of the problem E — [Ralsei](<https://codeforces.com/profile/Ralsei>))

## 1136E - Nastya Hasn't Written a Legend

Let $t_{i} = k_{1} + k_{2} + ... + k_{i - 1}$, $b_{i} = a_{i} - t_{i}$.

We can rewrite the condition $a_{i+1} \gt = a_{i} + k_{i}$, using array $b$:

$a_{i+1} \gt = a_{i} + k_{i}$

$a_{i+1} - k_{i} \gt = a_{i}$

$a_{i+1} - k_{i} - k_{i-1} - ... - k_{1} \gt = a_{i} - k_{i-1} - ... - k_{1}$

$a_{i+1} - t_{i+1} \gt = a_{i} - t_{i}$
...
Now let's find out what happens with $b$ after addition $x$ to position $i$. $b_{i}$ increases exactly on $x$. Then, if $b_{i+1} \lt b_{i}$, $b_{i+1}$ becomes equal to $b_{i}$, and so on for $i+2$, $i+3$, ..., $n$. Note that array $b$ is always sorted and each addition sets value $b_{i} + x$ in half-interval $[i, pos)$, where $pos$ - the lowest index such as $b_{pos} \gt = b_{i} + x$
