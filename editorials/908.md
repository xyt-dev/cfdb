...
Then, we have something like *dp*[*i*][*j*] = (*p*~*a*~ * *dp*[*i* + 1][*j*] + *p*~*b*~ * *dp*[*i*][*i* + *j*]) / (*p*~*a*~ + *p*~*b*~). The first term in this sum comes from adding another 'a' to our sequence, and the second comes from adding a 'b' to our sequence. We also have *dp*[*i*][*j*] = *j* if *j* ≥ *k* as a base case. The answer should be *dp*[0][0].
...
For an actual solution, partitions can be computed with bell numbers, which is an *O*(*m*^2^) dp (see this: [http://mathworld.wolfram.com/BellNumber.html](<http://mathworld.wolfram.com/BellNumber.html>) ).
