...
After drawing the rays from the first vertex (*n* - 2) triangles are formed. The subsequent rays will generate independently sub-regions in these triangles. Let's analyse the triangle determined by vertices 1, *i*, *i* + 1, after drawing the rays from vertex *i* and (*i* + 1) the triangle will be divided into (*n* - *i*) + (*i* - 2) = *n* - 2 regions. Therefore the total number of convex regions is (*n* - 2)<sup>2</sup>

![](eimages/592_1.png)

If the squirrel starts from the region that have 1 as a vertex, then she can go through each region of triangle (1, *i*, *i* + 1) once. That implies that the squirrel can collect all the walnuts in (*n* - 2)<sup>2</sup> jumps.
...
. Each of the remaining values *v*<sub>1</sub> - 1, *v*<sub>1</sub> - 2, ..., *v*<sub>2</sub> + 1 will appear exactly *M* times.
## Problem D. Super M
...
Let *S*<sub>*j*</sub> denote the number points that are between the rays *OR* and *jO*, then the number of triangles that have *i* as a vertex are

![](eimages/592_13.png)

. This summation can be calculated if we pre-calculate the cumulative sums of *S*<sub>*j*</sub>.
