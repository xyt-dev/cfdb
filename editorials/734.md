...
Let *k*~*a*~ will be amount of characters "A" in the string and *k*~*d*~ will be amount of characters "D" in the string. Then, if *k*~*a*~ > *k*~*d*~, we print "Anton". If *k*~*a*~ < *k*~*d*~, we print "Danik". If *k*~*a*~ = *k*~*d*~, we print "Friendship".
...
![](eimages/734_3.png)
 ...
 (we use *k*~2~ - *n*~256~ instead of *k*~2~, because *n*~256~ twos we've already used to make 256 numbers. Now it's not hard to observe that the answer will be equal to
...
Here *ba*~0~, *ba*~1~, ... mean the bits of *a* and *bb*~0~, *bb*~1~, ... mean the bits of *b*.
...
Let's create an array *d*~*i*~ where
...
Now it's not hard to find *a*~*i*~:

![](eimages/734_35.png)

Now, we only must check the answer for correctness. It's obvious then, if answer exists, it's alway unique, because it's explicitly derived from the formula above. To check if the answer exists, let's build arrays *b* and *c* from the found array *a* and compare it with the arrays given in the input. We'll do this separately for every bit. Let's calculate *k*~*j*~ — amount of numbers in array *a* that has a one in the *j*-th bit. Let's denote the *j*-th bit of *a*~*i*~ as *A*~*i*, *j*~. Now, let's count *B*~*i*, *j*~ and *C*~*i*, *j*~ such as
...
It's not hard to do since we know *k*~*j*~:

![](eimages/734_38.png)

See that if we calculate *B*~*i*, *j*~ и *C*~*i*, *j*~, it will be easy to find *b*~*i*~ и *c*~*i*~:
