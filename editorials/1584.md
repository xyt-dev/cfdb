<!-- url: https://codeforces.com/blog/entry/96953 -->
...
We know that the answer is integer, so if we manage to construct such a cut that it is necessary to color exactly such a $cnt$ of cells that$\frac{n \cdot m}{3} \leq cnt \lt \frac{n \cdot m}{3} + 1$, then $cnt$ will be the answer. After all, $cnt$ is the minimum integer value satisfying the estimate.
...
Now let's look on the number of inversions on subsegment [$x$, $n$]. If this number is less than A, then not both reversed subsegments fit entirely, so $i \lt x$, otherwise $i \geq x$.

Now we can apply binnary search to find $i$. We use $log_2(n)$ questions here.

Now let's ask the number of inversions on subsegment [$i+1$, $n$], let's call this number B. We use $1$ question here. From the structure of sequence: $A-B$ = $|\{x | x \gt i, a_x \lt a_i\}|$ = $|[i+1, j-1]|$ = $j - i - 1$.
...
If array contains only positive numbers, then it means that Bob is able to remove piles all the way over. Otherwise, let $t$ be the first moment with $c_t \lt 0$, this means that Bob was able to remove piles until he meet $t$-th pile and $c_{t-1} \gt a_t$ happened, so Bob loses. To check that last pile is empty, we need to check if $c_n = 0$.
...
- After that, we should avoid small blocks (with size $ \lt B$). Let's merge two consecutive blocks if one of them has the size $ \lt B$. After that, if the total block has size $\geq 2B$ let's split it into two equal blocks.
