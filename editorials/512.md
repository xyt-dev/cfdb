...
So we can transform the conditions *name*~1~ < *name*~2~, *name*~2~ < *name*~3~ ... into the order of letters.
...
First observation is that: *GCD*(*x*~1~, *x*~2~, ..., *x*~*k*~) = 1 means that for any prime *p*, there exist i such that *x*~*i*~ is not dividable by *p*. So we only care about what prime factors a number contain. (So for example, 12 -> {2, 3}, 6 -> {2, 3}, 9 -> {3]})
...
First finding is: if *a* + *b* is a prime, then one of them is an odd number, another is an even number. (that's why we set 2 ≤ *x*~*i*~)
...
Note: Actually this task is still solvable if we allow *a*~*i*~ = 1. But you need some clever way to deal with it. We think it is too hard so we removed this case. What do you think about this decision?
