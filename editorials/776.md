...
Editorial: We are going to loop over all possible non-negative powers of *k*. Since the maximum possible value of subarray sum can be 10<sup>5</sup> × 10<sup>9</sup> = 10<sup>14</sup>, there can be at most
...
So, we only need to consider the number of times we apply *f*() in *F*<sub>*k*</sub> which is (*k* + 1) / 2 for a given *k*.
...
Complexity: *O*(16 * 2<sup>16</sup> * 16 * 16)
