<!-- url: https://codeforces.com/blog/entry/67366 -->
...
Author: [kiyotaka](<https://codeforces.com/profile/kiyotaka>)

## 1148B - Born This Way

If $k \geq n$, we can cancel all flights between A and B, and Arkady won't be able to get to C, so answer will be equal to $-1$. Otherwise, $k \lt n$.
...
Now we can just iterate through all possible $x$ and calculate the biggest time moment. Since array $b$ is sorted, you can find $pos$ using binary search. The complexity of this solution is $O(n \log n)$. Also the array of $a_{x + 1} + t_a$ values is sorted too, so you can use two-pointers method. This solution will work with $O(n)$ complexity.
 Author: [KAN](<https://codeforces.com/profile/KAN>)
## 1148C - Crazy Diamond
...
One can be see, that the last case happens at most $\frac{n}{2}$ times, since when $i \geq \frac{n}{2} + 1$ you only need $3$ swaps or less. And so in total it's no more than $5 \cdot \frac{n}{2} + 3 \cdot \frac{n}{2} = 4 n$ swaps.
 Author: [Jatana](<https://codeforces.com/profile/Jatana>)
## 1148D - Dirty Deeds Done Dirt Cheap

First notice, that there are two types of pairs: one with $a_i \lt b_i$ and another one with $a_i \gt b_i$.

Note, that we can't form an answer with pairs of different types. Now let's notice that we can take all pairs of fixed type. Suppose the type is $a_i \lt b_i$. Then sort pairs by their $a_i$ in the decreasing order and that is the answer.

For type $a_i \gt b_i$ let's sort them by $b_i$ in increasing order.
...
However this is not enough, for example if $\delta_1 \lt 0$ (you need to move to the left the leftmost stone), there is clearly no answer. The real condition is that elements $\delta_i$ should form a "balanced bracket sequence", that is, the sum of every prefix of $\delta_i$ must be $\ge 0$. In this and only in this case we can match corresponding right-movements with left-movements.

In order to construct the exact answer we can go from left to right maintaining the stack of not-yet-closed "move-to-the right actions", formally we will put on stack pairs (stone_index, steps_to_move). So when we see stone with $\delta_i \gt 0$ we put it on the stack, and when we see stone with $\delta_i \lt 0$ we match it with the stones on the stack, removing the elements from the stack if they have moved to the full extent.
 Author: [Jatana](<https://codeforces.com/profile/Jatana>)
## 1148F - Foo Fighters
