...
## 350A - TL
Let's *v* = *min*(*a*<sub>*i*</sub>), *p* = *max*(*a*<sub>*i*</sub>), *c* = *min*(*b*<sub>*i*</sub>). So, if *max*(2 * *v*, *p*) < *c*, then answer is *max*(2 * *v*, *p*), else answer is  - 1.
...
## 350E - Wrong Floyd
Let's do the following: construct the graph with the maximum possible number of edges and then remove the excess. First of all, you can notice that if *k* = *n* answer is  - 1. Else let's fix some marked vertex, for example *a*<sub>1</sub>. Let's put in our graph all edges except edges beetween *a*<sub>1</sub> and *x*, where *x* — an another marked vertex.
